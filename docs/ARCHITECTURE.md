...
- One canonical bounded core-ecology patch whose local deer, gull, and black-bear populations derive from the seed, exact regional terrain, biome/climate signals, and a stable settlement-adjacent focus. Each species analysis may resolve to ecological absence. Present populations keep aggregate capacity, pressure, trend, and population units separate from a capped set of persistent representative actors with stable seed/region/population/ordinal identities, segmented positions, bounded dynamic state, and saved materialized/coarse state. Group-sized deer and gull representative populations also belong to stable group records; this remains one local assemblage rather than worldwide population generation.
...
This is not universal perception or a universal NPC architecture. The first porter-dog web and bounded deer/gull/black-bear assemblage extend the shared actor boundary through a narrow set of current consumers: one dog's physical food scent, wildlife visual contact, explicit anonymous alarm calls, bounded individual decisions, one exact physical scavenging transaction, and deer/gull signals inside their own persistent groups. General scent fields, environmental evidence and tracking, social reports and rumors, cross-group or broader species communication, physical human pursuit/search pathfinding, human-to-human sensing, generated people beyond the original estuary, additional dogs and worldwide wildlife populations, ownership and social networks, general physical NPC inventory, negotiation, deterrence, and companion behavior remain later slices.
...
`src/sim/coreWildlifeIdentity.ts` owns generation-v1 identities and profiles for deer, gull, and black bear. Identity derives from the root seed, signed origin region, semantic population key, population ordinal, species, and generation version—not array order or camera entry. Profiles supply bounded ecological roles, food affinities, alarm/flee/retreat/forage/guard thresholds, pursuit limit, morphs, temperament pairs, and individual vigilance/boldness/sociability ranges. `src/game/livingSpeciesRegistry.ts` is the lean runtime roster for human, dog, deer, gull, and black bear, providing ID namespace, presentation class, broad locomotion class, ABOUT noun, relative sensory values, and scent-range ceiling. `src/game/livingSpeciesCatalog.ts` gives those same five implemented species a strict versioned contract for identity, morphology, habitat status, population representation, senses, physiology, locomotion, diet/food-web role, life history, health boundary, activity, social state, cognition, evidence/aftermath status, interactions, environment, inventory, ABOUT, and persistence. Production alarm hearing and scent evaluation read the catalog's matching relative capabilities, while species-specific visual acuity remains unwired foundation data; roster drift or a missing sight/hearing/scent contract still fails immediately. The catalog contains no planned species. It now assigns the bounded habitat/population owner and deer/gull group owner without implying absent health, death, circadian, physical evidence, ecological migration/reproduction, or worldwide-habitat capabilities.

`src/game/livingSpeciesReleaseGate.ts` authenticates build-owned evidence for the same exact roster against 30 stable completeness criteria. A structurally valid caller claim cannot mark itself ready, absent behavior cannot smuggle an evidence owner, and biologically inapplicable criteria require narrow ecological proof. Habitat placement and bounded population materialization are now evidenced for all three wildlife profiles; same-species group behavior and a narrowly player-independent, nonlethal scenario are evidenced only for deer and gull. The current five species remain visibly incomplete for the broader biodiversity horizon where sound repertoires, physical environmental evidence, ecological migration/reproduction, worldwide scale, fuzzing, or cross-region wildlife behavior are not proven. This is a fail-closed development boundary, not a player statistic or a claim that one Alpha assemblage implements the eventual catalog.

`src/game/coreEcologyHabitat.ts` derives the Wave-A local assemblage directly from canonical regional terrain and its biome/climate projections. It evaluates a bounded stable focus, excludes settlement tiles, scores food, water, cover, nesting, climate, and predator pressure, and persists the complete derivation with its terrain hash. Every species receives an analysis even when its capacity is too low to support a population. Present populations store integer habitat capacity and population units, fixed-point pressure, a bounded trend signal/class, and deterministic habitat allocation anchors. At most eleven allocations represent those aggregate units, so visible actor count never pretends to equal the complete local population. Inputs are call-order independent and valid at signed extreme region addresses; the runtime rederives the expected habitat on load and rejects a mismatch instead of accepting a reroll.

`src/game/coreEcology.ts` owns the resulting version-2 patch, with hard limits of 12 populations, 48 exact representative members, and 24 materialized actors. Each member states how many aggregate units it represents, and canonicalization requires their sum to equal the population total. Full simulation continues to use exact actors and lawful observations. A coarse actor ages needs, condition, and perception with an empty observation frame; it applies only the remainder of an already-authoritative intent until that intent expires, then returns to neutral observation. Coarse advancement cannot invent a sighting, target, food opportunity, resource claim, movement, or new decision.
...
`src/game/coreEcologyPerception.ts` converts current in-window visual contacts into the same classified observation vocabulary used by living actors. Terrain, structures, facing, weather visibility, and static target light affect current visual contact; target movement salience and species-specific visual acuity are not yet connected for this crossing. Bear is classified as predator to deer, gull, dog, and human; deer is classified as live prey only to bear. An alarm is emitted only by a committed actor event and propagated through bounded, weather/wind-modified hearing, including species hearing sensitivity, as an anonymous approximate area. It carries no emitter ID/species, hidden target, or internal motive. The active set is tightly capped; broader density still requires spatial buckets instead of all-pairs scanning.

`src/game/coreWildlifeActor.ts` turns accepted observations, bounded needs/condition, generated temperament, role affinities, and action accessibility into alarm, flee, retreat, guard, scavenge, forage, bounded pursue, rest, observe, or disengage. A proposal never mutates another actor or item. Non-gull movement uses the shared traversability resolver: standable shallow water remains open and is now covered by an end-to-end threatened-deer movement fixture, while water deeper than the existing stand threshold rejects the route. Gull movement uses bounded aerial steps. This slice adds no harmful contact, injury, death, carcass, hunting, ecological migration/reproduction, or reward loop.

When the habitat assemblage contains a bear, the runtime seeds one exact loose dried-fish parcel near it. Visual evidence can make that parcel a food opportunity, but only an identified, directly confirmed, accessible whole unit may produce a claim. The cargo owner rechecks exact segmented contact, payload kind, quantity, and current existence before atomically tombstoning that same entity, recording `animal-consumption`, and reducing hunger. A replay, malformed claim, partial stack, consumed item, or out-of-reach seam case leaves both cargo and ecology unchanged. Player-facing consumption narration is emitted only when the event-time actor was directly visible; otherwise authoritative history remains silent to the player. A newly heard alarm may still receive its deliberately anonymous, non-identifying caption. Persisted group aftermath is likewise not a report: it requires a future lawful direct-observation consumer before it can become player knowledge.

Chart and Relief project the same direct-detail wildlife set and use species plus stable ID for selection. ABOUT may disclose only currently observable classification, visible group estimate, condition wording, behavior, morph, and life stage at sufficient clarity. It never exposes stable IDs, raw needs/traits, target, causal reference, exact hidden population count, or a retroactive offscreen event. The only wildlife choices are **WAIT AND WATCH**, **ROUTE AROUND THIS SPOT**, and **LEAVE**; those reuse the existing porter policy rather than commanding an animal.
...
2. `tideweft-session` contains the serialized world plus player motion/cargo/report/chart/Wayknot state, tutorial state, chosen posture, a legacy-compatible session-shape field, recap history, a sealed pending-perception carry, the first living web's dog ecology, porter response, player-choice state, and the canonical core-ecology patch. The outer session is version 9. Versions 1 through 4 migrate with an empty sensory carry; version 5 preserves its partial fixed-step phase and bounded sensory suffix; version 6 preserves its existing dog ecology while deterministically initializing porter-response and empty player-choice state; and version 7 retains those established migrations before receiving current seed-derived local habitat state without invented prior ecology history. Exact version-8 sessions accept only their canonical version-1 fixed ecology topology and migrate it once to the version-2 representation: actor identities, positions, physiology, cognition, event history, materialization, and the already-persisted physical cargo stay exact; each legacy actor represents one unit; derivation is marked `legacy-fixed-v1`; and no habitat or group history is invented. A version-9 envelope requires the current version-2 ecology shape and authenticates its persisted habitat, expected group topology, actor identities, cargo roots, envelope integrity, and cross-ledger conservation. Its sealed regional-travel payload remains version 2 and stores the exact global origin of the 120 × 120 presentation frame; valid version-1 98 × 74 payloads migrate into a player-centered frame without moving the courier or changing chart knowledge. It does not serialize derived Tide Harps or biome profiles.
...
`src/ui/tutorialGuide.ts` is platform-neutral data with guide version 24, stable section/control IDs, audience filters, and explicit live/planned status. Its eighteen topics include the unbroken-world contract, original-estuary human identity and perception, the one-dog porter web, the bounded deer/gull/black-bear crossing, the shared five-species sensory contract, OBSERVED-versus-KNOWN, GREET, and non-pausing ABOUT alongside premise, movement, Promises, reports, traversal, fieldcraft, routes, views/HUD, saves/Quiet Hour, accessibility, and build boundaries. The living-actor lesson explains lawful sight/hearing, physical food scent, anonymous alarms, rain/condition, the request-versus-command boundary, real WAIT/REROUTE/LEAVE choices, exact whole-parcel consumption, memory/persistence, and the absence of a full animal roster, combat, or companion system. The player-facing world lesson uses continuous E/N coordinates and states that no edge action, generation prompt, address banner, loading screen, or second click is required. Every completed feature phase must update this source and its focused tests before the mechanic can be considered explained.
...
11. Core-wildlife same-seed identity, profile/registry coherence, habitat hashes and honest absence, aggregate capacity/pressure/trend conservation, representative/group/materialization caps, array-order independence, direct/peripheral/occluded sight, anonymous alarm and group-signal propagation, herd/flock cohesion and split/rejoin lineage, predator/prey/easier-food decisions, bounded pursuit/disengagement, standable-shallow-water movement, whole-parcel conservation, negative seam reach, nonlethal cargo-neutral player-absent aftermath, full/coarse/full continuity, exact outer-v8 migration/current-v9 reload, and knowledge-safe Chart/Relief ABOUT.
