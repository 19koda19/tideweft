...
- One canonical bounded core-ecology patch containing two deer, three gulls, and one black bear. Its exact members have stable seed/region/population/ordinal identities, segmented positions, bounded dynamic state, and a saved materialized/coarse flag; this is one local crossing rather than general habitat-derived world population.
...
This is not universal perception or a universal NPC architecture. The first porter-dog web and bounded deer/gull/black-bear crossing extend the shared actor boundary through a narrow set of current consumers: one dog's physical food scent, wildlife visual contact, explicit anonymous alarm calls, bounded individual decisions, and one exact physical scavenging transaction. General scent fields, environmental evidence and tracking, social reports and rumors, group communication, physical human pursuit/search pathfinding, human-to-human sensing, generated people beyond the original estuary, additional dogs and worldwide wildlife populations, persistent herd/flock social state, ownership and social networks, general physical NPC inventory, negotiation, deterrence, and companion behavior remain later slices.
...
## Bounded core-wildlife crossing

`src/sim/coreWildlifeIdentity.ts` owns generation-v1 identities and profiles for deer, gull, and black bear. Identity derives from the root seed, signed origin region, semantic population key, population ordinal, species, and generation version—not array order or camera entry. Profiles supply bounded ecological roles, food affinities, alarm/flee/retreat/forage/guard thresholds, pursuit limit, morphs, temperament pairs, and individual vigilance/boldness/sociability ranges. `src/game/livingSpeciesRegistry.ts` is the lean runtime roster for human, dog, deer, gull, and black bear, providing ID namespace, presentation class, broad locomotion class, ABOUT noun, relative sensory values, and scent-range ceiling. `src/game/livingSpeciesCatalog.ts` now gives those same five implemented species a strict versioned contract for identity, morphology, habitat status, population representation, senses, physiology, locomotion, diet/food-web role, life history, health boundary, activity, social state, cognition, evidence/aftermath status, interactions, environment, inventory, ABOUT, and persistence. Production alarm hearing and scent evaluation read the catalog's matching relative capabilities, while species-specific visual acuity remains unwired foundation data; roster drift or a missing sight/hearing/scent contract still fails immediately. The catalog contains no planned species and explicitly leaves absent health, death, circadian, evidence, group, and worldwide-habitat capabilities unimplemented rather than implying them from data.

`src/game/livingSpeciesReleaseGate.ts` authenticates build-owned evidence for the same exact roster against 30 stable completeness criteria. A structurally valid caller claim cannot mark itself ready, absent behavior cannot smuggle an evidence owner, and biologically inapplicable criteria require narrow ecological proof. The current five species remain visibly incomplete for broader biodiversity release where sound repertoires, same-species/group behavior, environmental evidence, player-independent ecology, measured scale, or cross-region behavior are not proven. This is a fail-closed development boundary, not a player statistic or a claim that the bounded Alpha crossing implements the entire catalog horizon.

`src/game/coreEcology.ts` owns one bounded patch of at most 12 populations, 48 exact members, and 24 materialized actors. Alpha 12 instantiates a fixed local crossing beside the porter-dog web with two deer, three gulls, and one hungry black bear. Every member currently remains an exact individual record even while marked coarse; camera-window materialization changes representation without recreating identity or dynamic state. Coarse members do not think or move. The patch does not yet derive habitat capacity, population pressure, trend, migration, reproduction, persistent herd/flock cohesion, or a worldwide assemblage from terrain.
...
11. Core-wildlife same-seed identity, profile/registry coherence, patch/member/materialization caps, array-order independence, direct/peripheral/occluded sight, anonymous alarm propagation, predator/prey/easier-food decisions, bounded pursuit/disengagement, whole-parcel conservation, negative seam reach, full/coarse/full continuity, save migration/reload, and knowledge-safe Chart/Relief ABOUT.
