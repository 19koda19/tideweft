...
- One canonical bounded core-ecology patch whose local deer, gull, black-bear, brown-rat, and domestic-cat populations derive from the seed, exact regional terrain, biome/climate signals, and a stable settlement-adjacent focus. Each species analysis may resolve to ecological absence. Deer, gulls, bears, and free-ranging cats use capped persistent individual representatives with stable seed/region/population/ordinal identities, segmented positions, bounded dynamic state, and saved materialized/coarse state. Brown rats instead use one stable population-area aggregate with bounded habitat anchors, evidence, and disturbance history; no individual rat actors are synthesized. Group-sized deer and gull populations also belong to stable group records. This remains one local assemblage rather than worldwide population generation.
...
This is not universal perception or a universal NPC architecture. The first porter-dog web and bounded harbor-edge assemblage extend the shared boundary through a narrow set of current consumers: one dog's physical food scent, individual-wildlife visual contact, explicit anonymous alarm calls, bounded individual decisions, one exact physical scavenging transaction, deer/gull signals inside persistent groups, and brown-rat aggregate pressure derived from lawful visual contacts, loose-provision scent, rain, and terrain exposure. Directly observable rat signs and rain-response cat pawprints are the only current physical-evidence consumers. General scent fields, broad evidence and tracking, social reports and rumors, cross-group or broader species communication, physical human pursuit/search pathfinding, human-to-human sensing, generated people beyond the original estuary, additional dogs and worldwide wildlife populations, ownership and social networks, general physical NPC inventory, negotiation, deterrence, and companion behavior remain later slices.
...
`src/sim/coreWildlifeIdentity.ts` owns generation-v1 profiles for deer, gull, black bear, brown rat, and domestic cat. Persistent individual identity applies to deer, gulls, bears, and cats; brown-rat metadata explicitly selects aggregate representation, so it cannot pass through the individual actor constructor. Individual identity derives from the root seed, signed origin region, semantic population key, population ordinal, species, and generation version—not array order or camera entry. Profiles supply bounded ecological roles, food affinities, behavior thresholds, morphs, temperament pairs, and individual trait ranges where those concepts apply. `src/game/livingSpeciesRegistry.ts` is the lean runtime roster for seven current species records: human, domestic dog, deer, gull, black bear, brown rat, and domestic cat. It owns representation and addressability as well as presentation, locomotion, ABOUT nouns, and relative senses. `src/game/livingSpeciesCatalog.ts` gives the same exact roster a strict versioned contract without treating a rat population area as an actor. Roster drift, an addressable aggregate, or a missing required sense contract fails closed. The catalog contains no planned species and does not imply absent health, death, circadian, ecological migration/reproduction, or worldwide-habitat capabilities.

`src/game/livingSpeciesReleaseGate.ts` authenticates build-owned evidence for that exact roster against 30 stable completeness criteria. A structurally valid caller claim cannot mark itself ready, absent behavior cannot smuggle an evidence owner, and biologically inapplicable criteria require narrow ecological proof. The rat and cat records may claim only the connected Settlement Shadows owners: habitat, aggregate or individual persistence, lawful sensory input, bounded movement, sound, narrow evidence, ABOUT, and representative interaction tests. Aggregate density spacing and directly observed cat food competition provide one bounded same-species outcome for each representation without claiming a quadratic animal-pair matrix. Broad food-web turnover, complete multisensory coverage, health/death, worldwide ecology, rat migration across region anchors, and exact deployed-release evidence stay blocked where absent. This is a fail-closed development boundary, not a player statistic or a claim that one harbor-edge assemblage implements the eventual catalog.

`src/game/coreEcologyHabitat.ts` preserves the frozen Wave-A analysis for deer, gulls, and black bears, then applies a version-2 harbor-edge extension for brown rats and domestic cats without rerolling those earlier populations. It evaluates a bounded stable focus, excludes settlement tiles, scores food, water, cover, nesting or shelter, climate, settlement influence, and predator pressure, and persists the complete derivation with its terrain hash. Every species receives an analysis even when its capacity is too low to support a population. Present populations store integer habitat capacity and population units, fixed-point pressure, a bounded trend signal/class, and deterministic allocation anchors. Rat areas use a separate occupancy plane so a dense aggregate cannot evict individual wildlife or create dozens of actor identities. Inputs are call-order independent and valid at signed extreme region addresses; the runtime rederives the expected habitat on load and rejects a mismatch instead of accepting a reroll.

`src/game/coreEcology.ts` owns the resulting version-3 aggregate-capable patch, with hard limits of 12 individual populations, 48 exact representative members, 24 materialized actors, four aggregate populations, four anchors per aggregate, 24 retained evidence records, and 16 retained disturbances. Each individual representative states how many population units it represents, and canonicalization requires their sum to equal the population total. A brown-rat aggregate separately conserves its population units across its anchors. Full simulation continues to use exact actors and lawful observations. A coarse individual ages needs, condition, and perception with an empty observation frame; it applies only the remainder of an already-authoritative intent until that intent expires, then returns to neutral observation. Coarse advancement cannot invent a sighting, target, food opportunity, resource claim, movement, or new decision.

`src/game/coreEcologyGroups.ts` adds one persistent herd or flock record when a derived deer or gull population has at least two representatives; black bears remain solitary. A group stores stable identity and membership, cohesion, phase, movement heading, one or two component anchors, bounded anonymous signal propagation, split/rejoin lineage, and bounded aftermath. Signals reach additional members only on exact coarse cadence rather than setting every hidden target at once. When every group member is coarse, sufficiently high persisted habitat pressure may authorize a bounded displacement or split between existing habitat allocation anchors. That transition is explicitly player-absent, nonlethal, cargo-neutral, and unavailable as automatic player knowledge. Cohesion can later produce a saved reunion. Full-to-coarse reconciliation copies lawful member positions into the group anchors; coarse-to-full return reuses the same actor IDs and places members around their saved component anchor without duplication.

`src/game/coreEcologyPerception.ts` converts current in-window visual contacts into the same classified observation vocabulary used by living actors. Terrain, structures, facing, weather visibility, and static target light affect current visual contact; target movement salience and species-specific visual acuity are still narrow inputs rather than a complete universal sensory field. Bears remain predators to deer, gulls, dogs, and humans; cats, dogs, people, and gulls can also participate in lawful shared visual contact around the rat aggregate. An alarm is emitted only by a committed actor event and propagated through bounded, weather/wind-modified hearing as an anonymous approximate area. It carries no emitter ID/species, hidden target, or internal motive. The active set is tightly capped; broader density still requires spatial buckets instead of all-pairs scanning.

`src/game/coreEcologyAggregatePerception.ts` is the sole runtime bridge from current world truth into the brown-rat aggregate. It resolves per-anchor visual contact through the existing terrain/weather occlusion rules; derives attraction from currently existing loose provision objects through the shared wind- and rain-shaped scent evaluator; and derives rain pressure from authoritative weather plus each saved anchor's terrain exposure. It submits a bounded canonical stimulus frame rather than giving the aggregate kernel access to actor lists, player state, weather, or cargo. `src/game/coreEcologySmallWorld.ts` then deterministically selects at most one one-unit redistribution per aggregate on an eight-tick cadence. When no stronger external relocation exists and saved anchor populations differ by at least two units, the same kernel may space one conserved unit toward a quieter anchor; the indivisible unit cannot oscillate between equal anchors. Every event explicitly records no mortality, no cargo interaction, no item consumption, and no player knowledge. This is an aggregate population response, not individual rat cognition or pathfinding.

`src/game/coreWildlifeActor.ts` turns accepted observations, bounded needs/condition, generated temperament, role affinities, and action accessibility into alarm, flee, retreat, guard, scavenge, forage, bounded pursue, rest, observe, or disengage. A proposal never mutates another actor or item. A directly observed second cat can supply food-competition pressure that changes a hungry cat's selected provision response from forage to guard. Strong local rain supplies one anonymous local cue to a materialized cat; sufficient pressure produces a routed retreat and a rate-limited wet-track record without adding injury or exposure physiology. Non-gull movement uses the shared traversability resolver: standable shallow water remains open and is now covered by an end-to-end threatened-deer movement fixture, while water deeper than the existing stand threshold rejects the route. Gull movement uses bounded aerial steps. This slice adds no harmful contact, injury, death, carcass, hunting, ecological migration/reproduction, or reward loop.

When the habitat assemblage contains a bear, the runtime seeds one exact loose dried-fish parcel near it. Visual evidence can make that parcel a food opportunity, but only an identified, directly confirmed, accessible whole unit may produce a claim. The cargo owner rechecks exact segmented contact, payload kind, quantity, and current existence before atomically tombstoning that same entity, recording `animal-consumption`, and reducing hunger. A replay, malformed claim, partial stack, consumed item, or out-of-reach seam case leaves both cargo and ecology unchanged. The Settlement Shadows scent bridge reads only loose provisions that remain after this custody step. Rat attraction never consumes, moves, aliases, or duplicates one; it changes only the distribution of a conserved aggregate population. Player-facing consumption narration is emitted only when the event-time actor was directly visible; otherwise authoritative history remains silent to the player.

Chart and Relief project the same direct-detail individual wildlife set and use species plus stable ID for selection. Domestic cats receive their own low-cost form and the ordinary wildlife choices: **WAIT AND WATCH**, **ROUTE AROUND THIS SPOT**, and **LEAVE**. They are neither owned nor recruitable. Brown-rat evidence uses a separate target type keyed by aggregate and evidence identity; gnaw marks, small tracks, or shelter signs render and open a close-only ABOUT surface only under current direct-detail sight. Saved wet cat pawprints pass through the same direct-detail projection but remain non-targetable, disclose no causal memory, and never substitute for sight of the living cat. ABOUT may disclose observable classification, evidence kind, condition wording, behavior, appearance, or life stage where supported, but never stable IDs, raw needs/traits, causal references, exact hidden population count, activity pressure, anchors, or retroactive offscreen events. Both paths share mouse/touch and Chart/Relief commands, and loss of sight clears the ephemeral target.

`src/audio/soundscape.ts` adds deterministic rat-rustle and domestic-cat-call patterns. The runtime plays them only when the causative aggregate evidence or cat intent event is visible at event time. They are redundant presentation cues, not simulated long-range sound sources, social communication, or permission to reveal hidden activity.
...
2. `tideweft-session` contains the serialized world plus player motion/cargo/report/chart/Wayknot state, tutorial state, chosen posture, a legacy-compatible session-shape field, recap history, a sealed pending-perception carry, the first living web's dog ecology, porter response, player-choice state, and the canonical core-ecology patch. The outer session is version 10. Versions 1 through 8 retain their established deterministic migrations into the Wave-A habitat/group shape. An exact version-9 envelope must first authenticate that canonical version-2 ecology record, its actors and groups, the physical cargo roots, envelope integrity, and cross-ledger conservation. Migration then preserves every existing actor identity and state, group, item, custody record, Promise, and world fact while deriving the version-2 harbor-edge habitat once and adding its seed-bound brown-rat aggregate and domestic-cat individuals. A version-10 envelope requires the aggregate-capable version-3 ecology shape and authenticates its habitat derivation, aggregate unit conservation, evidence and disturbance bounds, actor identities, group topology, cargo roots, and cross-ledger conservation; it cannot run the adoption twice. Its sealed regional-travel payload remains version 2 and stores the exact global origin of the 120 × 120 presentation frame; valid version-1 98 × 74 payloads migrate into a player-centered frame without moving the courier or changing chart knowledge. It does not serialize derived Tide Harps or biome profiles.
...
`src/ui/tutorialGuide.ts` is platform-neutral data with guide version 25, stable section/control IDs, audience filters, and explicit live/planned status. Its eighteen topics include the unbroken-world contract, original-estuary human identity and perception, the one-dog porter web, the bounded harbor-edge assemblage, the seven-record species catalog, OBSERVED-versus-KNOWN, GREET, and non-pausing ABOUT alongside premise, movement, Promises, reports, traversal, fieldcraft, routes, views/HUD, saves/Quiet Hour, accessibility, and build boundaries. The living-actor lesson distinguishes individual cats from aggregate brown-rat population areas; explains direct rat signs, visible-event-only sound cues, lawful animal/food/weather pressure, bounded redistribution with no consumption, full/coarse identity, and version-10 adoption; and retains the request-versus-command and physical-custody boundaries. It also names the absence of a full animal roster, combat, hunting, cat ownership, or companion system. The player-facing world lesson uses continuous E/N coordinates and states that no edge action, generation prompt, address banner, loading screen, or second click is required. Every completed feature phase must update this source and its focused tests before the mechanic can be considered explained.
...
11. Core-wildlife same-seed identity, profile/registry coherence, habitat hashes and honest absence, individual and aggregate population-unit conservation, representative/group/materialization/evidence/disturbance caps, array-order independence, direct/peripheral/occluded sight, anonymous alarm and group-signal propagation, herd/flock cohesion and split/rejoin lineage, predator/prey/easier-food decisions, cat/dog/human/gull lawful visual contact, rat food-scent and rain/terrain pressure, bounded aggregate redistribution, whole-parcel conservation, negative seam reach, nonlethal cargo-neutral player-absent aftermath, full/coarse/full continuity, exact outer-v9 migration/current-v10 reload, and knowledge-safe Chart/Relief individual/evidence ABOUT.
12. Vite production build under relative paths.
13. Packaged Electron launch, visible title controls, `app://` resource load, preserved-estuary content inside the 120 × 120 moving frame, deterministic R1/A3/W5 Harp placement and remote echo, both Chart/Relief canvas switches, actual Relief bell/cord evidence, desktop plus portrait/landscape mobile probes, Node-global absence, and zero renderer warnings/resource failures.
