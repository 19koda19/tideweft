...
The current `0.3.3-alpha.30 — Beyond the Harbor` repository state is a source
candidate, not a **LIVE_VERIFIED** deployment. It extends the catalog to 22
...
Dog interaction is an intentional no-response, and exact deployment remains
unattested.
...
records. Existing mortality and finite-body history remains byte-for-byte
...
perception and locomotion owners, but it does not widen the harmful-contact
contract: only a solitary, addressable marsh rabbit can enter the current
exact-contact mortality transaction. Grouped elk, grouped deer, and every
other group member fail closed for harm. A wild boar can lawfully see, reach,
claim, and consume an already-existing body through the same finite-resource
conservation seam. The three new species have authored voice-pattern data but
...
crows, and wild boars, with guarding limited to the fox policy; it does not
...
The Alpha 30 Beyond the Harbor source candidate advances the outer session to
...
This paragraph describes repository source, not a verified deployment.
