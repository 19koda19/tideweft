...
- One canonical bounded core-ecology patch whose local deer, gull, black-bear, brown-rat, domestic-cat, marsh-rabbit, marsh-fox, fish-crow, northern-harrier, southern-leopard-frog, Atlantic-silverside, Atlantic-marsh-fiddler-crab, snowy-egret, American-black-duck, and North American river otter populations derive from the seed, exact regional terrain, biome/climate signals, and a stable settlement-adjacent focus. Each species analysis may resolve to ecological absence. Deer, gulls, bears, free-ranging cats, rabbits, foxes, fish crows, northern harriers, snowy egrets, American black ducks, and river otters use capped persistent individual representatives with stable seed/region/population/ordinal identities, segmented positions, bounded dynamic state, and saved materialized/coarse state. Fish crows use at most three visible representatives and a stable flock record when grouping is viable; northern harriers, snowy egrets, American black ducks, and river otters each expose at most one representative. The otter requires both tidal aggregate populations plus authenticated usable water and a distinct dry haulout. Brown rats, southern leopard frogs, Atlantic silversides, and Atlantic marsh fiddler crabs instead use stable aggregate populations; no fish or crab actor is synthesized. This remains one local assemblage rather than worldwide population generation or a complete bestiary.
...
`src/game/coreWildlifeActor.ts` turns accepted observations, bounded needs/condition, generated temperament, role affinities, runtime capabilities, and action accessibility into alarm, flee, retreat, guard, scavenge, forage, bounded pursue, rest, observe, or disengage. A proposal never mutates another actor or item. Rabbits retain their causal alarm and flight; hungry foxes retain finite small-prey pursuit and pressure-based interruption. A fish crow can alarm at a directly confirmed harrier, and a hungry northern harrier can pursue directly confirmed small prey until lost opportunity, stronger pressure, or crow mobbing makes it disengage. A directly observed second cat can still supply food-competition pressure that changes a hungry cat's selected provision response from forage to guard. Strong local rain supplies one anonymous local cue to a materialized cat; sufficient pressure produces a routed retreat and a rate-limited wet-track record without adding injury or exposure physiology.
...
2. `tideweft-session` contains the serialized world plus player motion/cargo/report/chart/Wayknot state, tutorial state, chosen posture, a legacy-compatible session-shape field, recap history, a sealed pending-perception carry, the first living web's dog ecology, porter response, player-choice state, the canonical core-ecology patch, and the settlement-ecology root. The Alpha-25 source candidate advances the outer session to version 18, habitat analysis to version 9, and settlement ecology to version 3 while aggregate ecology remains schema version 4. A sealed version-17 Yard Flock envelope is authenticated before one pen, exactly two goats, one herd, and one additional domestic-custody relationship are appended. Every habitat-8 population and anchor, actor, group, aggregate unit, item, Promise, evidence record, store fact, world fact, chicken member, flock, coop, keeper, and physical food-use transaction remains exact. Version 18 persists plural typed domestic homes and stable goat identity; reload cannot reroll either domestic population, reuse a group or structure, duplicate a provision, replay a meal, or grant goats access to the chickens' store opportunity. Versions 1 through 16 continue through their established frozen one-way migrations before this final adoption. Spatial top-K materialization remains a deterministic projection from local distance with stable actor identity breaking ties and does not rewrite authoritative coarse state. The sealed regional-travel payload remains version 2 and stores the exact global origin of the 120 × 120 presentation frame; valid version-1 98 × 74 payloads migrate into a player-centered frame without moving the courier or changing chart knowledge. It does not serialize derived Tide Harps, biome profiles, target-tick tidal depths, or the current materialized-detail selection.
...
`src/ui/tutorialGuide.ts` is platform-neutral data with stable section/control IDs, audience filters, and explicit live/planned status. Guide versions 26 through 32 preserve the verified Alpha-16 through Alpha-22 lessons, while released version 33 records the Storehouse Door and outer-v15-to-v16 adoption. Candidate version 34 adds the one stable two-to-three-chicken yard flock, individual/group/custody identities, shared perception and terrestrial movement, broad ecological roles, exact open-store food transaction, secured-store and event-time knowledge boundaries, habitat-8 exact-prefix rule, settlement ecology 2, and outer-v16-to-v17 adoption. It explicitly names worldwide livestock, calls/tracks, eggs, nesting, reproduction, schedules, autonomous home return, guardian/herding behavior, harmful attack, injury, mortality, carcasses, live-prey consumption, the full bestiary, full Wave D, Directive 04_1 completion, and exhaustive species/pair testing as absent. The player-facing world lesson uses continuous E/N coordinates and states that no edge action, generation prompt, address banner, loading screen, or second click is required.
...
Source version `0.3.3-alpha.25 — The Far Paddock` extends that bounded domestic
...
still absent from this source candidate.
