...
- One canonical bounded core-ecology patch whose exact habitat-v9 prefix derives the starting-harbor deer, gull, black-bear, brown-rat, domestic-cat, domestic-chicken, domestic-goat, marsh-rabbit, marsh-fox, fish-crow, northern-harrier, southern-leopard-frog, Atlantic-silverside, Atlantic-marsh-fiddler-crab, snowy-egret, American-black-duck, and North American river otter populations. Habitat v10 retains that prefix and appends one deterministic remote temperate-upland/forest-edge source for wild boar, elk, and gray wolf. Every wild-species analysis derives from the seed, exact regional terrain, and biome/climate signals and may resolve to honest ecological absence. Addressable species use capped persistent representatives with stable seed/region/population/ordinal identities, segmented positions, bounded dynamic state, and saved materialized/coarse state. The starting settlement owns one stable two-to-three-chicken flock in a coop and one stable two-goat herd in a separate pen; eligible crows, boars, elk, and wolves use the shared flock, sounder, herd, and pack topology. Northern harriers, snowy egrets, American black ducks, and river otters each expose at most one representative, and the otter requires both tidal aggregate populations plus authenticated usable water and a distinct dry haulout. Brown rats, southern leopard frogs, Atlantic silversides, and Atlantic marsh fiddler crabs instead use stable aggregate populations; no fish or crab actor is synthesized. These are two bounded sources rather than worldwide population generation or a complete bestiary.
...
The current `0.3.3-alpha.30 — Beyond the Harbor` release is
**LIVE_VERIFIED**. It extends the catalog to 22
records with wild boar, elk, and gray wolf in one deterministic remote
temperate-upland/forest-edge source. The release gate gives active owners only
...
Dog interaction is an intentional no-response.
...
Habitat analysis version 10 preserves the complete version-9 population and
anchor record as an exact prefix, then appends the three regional populations
and their one bounded source. Outer session version 23 likewise authenticates
and adopts a sealed version-22 What Remains payload exactly once before adding
the regional source, allocations, actors, and `SOUNDER` / `HERD` / `PACK`
records. Existing mortality and finite-body history remain byte-for-byte
authoritative through adoption. The core-ecology patch remains version 3 and
its aggregate record remains version 5.

Gray-wolf role appraisal may produce pursuit pressure through the shared
...
`src/game/coreWildlifeActor.ts` turns accepted observations, bounded needs/condition, generated temperament, role affinities, runtime capabilities, and action accessibility into alarm, flee, retreat, guard, scavenge, forage, bounded pursue, rest, observe, or disengage. A proposal never mutates another actor or item. Rabbits retain causal alarm and flight; hungry foxes and gray wolves retain finite prey pursuit and pressure-based interruption. Only the separate mortality resolver may translate an extant current fox or wolf pursuit plus exact eligible-rabbit contact into damage. Fish-crow alarm/mobbing, nonlethal harrier pursuit, cat food competition, and rain retreat retain their prior bounded rules.
...
permits a marsh fox or gray wolf against an individually represented solitary
...
crows, wild boars, and gray wolves, with guarding available only to fox and
wolf policy; it does not
...
cougar, additional bear ecotypes, worldwide ecology, full Wave E, and
