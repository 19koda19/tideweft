...
Design implication: future worldwide populations should expand this aggregate/representative boundary rather than multiplying full actors. Ecological confidence should come from shared invariants, conservation, deterministic outcome classes, representative encounters, and bounded interaction-graph fuzzing—not a brittle bespoke test for every animal pair. Broader species, reproduction, ecological migration, carcasses, circadian behavior, general scent fields, and broad evidence tracking still require their own authoritative owners and performance proof; this local rat-sign consumer does not stand in for them.
