...
- One bounded local core-wildlife assemblage attached to that first living web. Deer, gull, and black-bear presence derives from the stable seed, terrain, biome/climate signals, and a settlement-adjacent focus, so unsuitable local ecology can contain no member of a species. Present populations retain aggregate capacity, pressure, trend, and units separately from persistent representative actors. Group-sized deer and gull representative populations also retain herd/flock state; this is not worldwide habitat generation.
...
Habitat analysis stores carrying capacity inputs, aggregate population units, pressure, trend, and deterministic placement anchors even though only a bounded set of representatives materializes. Empty population is a valid ecological result, not a failed spawn. Group-sized deer and gull representative populations receive stable herd/flock membership, cohesion, group components, anonymous alarm-signal state, and split/rejoin lineage; black bears remain solitary. When an entire group is coarse, high persisted habitat pressure can produce a bounded player-absent displacement or separation between already validated habitat anchors, followed later by reunion as cohesion recovers. This aftermath is explicitly nonlethal, cannot touch cargo, and never enters player EVENTS or knowledge automatically; player-facing aftermath inspection is not live.
...
- Outer save format 9 preserves the sealed partial fixed-step phase and bounded player sensory samples introduced by version 5, plus dog ecology, porter response, player-choice history, provision custody, bounded memory, promotion, and the current habitat/population/group ecology record. Exact version-8 sessions migrate their canonical fixed ecology once without rerolling any existing actor or physical cargo: actor state remains exact, each old actor represents one unit, and the migrated patch is explicitly legacy-fixed without invented habitat or group history. A current version-9 envelope must already contain the authenticated version-2 ecology shape. Committed human cognition remains in embedded simulation format 4 and `tideweft-sim/6`.
...
- one versioned production species contract covering exactly the five implemented living-actor types—human, domestic dog, deer, gull, and black bear—whose sensory data feeds the shared evaluator and whose bounded wildlife habitat/population and deer/gull group owners are now explicit; general evidence, health/death, circadian behavior, worldwide habitat breadth, ecological migration/reproduction, broad fuzzing, and measured release-scale capabilities remain closed;
