...
Portrait, short-landscape, and desktop layouts now use the same pane-free field facts over the world: terrain and biome, WATER or GROUND and known depth, effort, current stability percentage and cause, signed region/local/global coordinates, and measured FPS. **PROMISES + / PROMISES −** opens a mutually exclusive safe-area workspace on compact screens. Discovered wet surface in Chart and Relief carries moving streamlines and sparse foam; SOUND / SCAN adds analytical arrowheads, while exact unsounded depth remains private. In water at or above **120,000** depth, either empty stamina or empty current stability enters the same controllable ADRIFT state on keyboard and touch.
...
Relief 3D travel is camera-relative: after orbiting with J/L, drag, or a two-finger twist, WASD/arrows continue to mean screen-left, screen-right, forward, and back. A pointer-transparent N compass stays north-up in Chart and turns with the Relief camera so world north remains legible without changing any simulation direction. Terrain that has just left broad sight keeps a sub-second visual impression before easing into dim durable Chart memory or uncharted darkness, and the same bounded scalar buffer follows a quick Chart/Relief switch. Exact people, parcels, resources, water detail, labels, actions, and pointer targets remain live-gated and disappear immediately; no perception memory is serialized.
...
The `0.3.3-alpha.7` source candidate carries gameplay contract 15 and tutorial 17. It makes stability a direct terrain-and-force percentage, gives local rivers deterministic calm/rough flow character, limits analytical arrows to SOUND / SCAN, removes weather-driven world shake, and keeps Relief's dry ground earthy on the first and every later frame by resetting leaked WebGL emissive state. Desktop, portrait, and landscape expose the same terrain, depth, effort, stability, signed-coordinate, and measured-FPS facts without panes. The wide terrain/short exact-detail perception split now gives only terrain a brief eased visual memory shared across Chart and Relief; exact entities and interactions still cut off immediately. Player-controlled ADRIFT recovery and physical cargo custody remain intact. [CHANGELOG.md](./CHANGELOG.md), title and Quiet Hour Patch Notes, and the field manual's **What's New** page are generated from or synchronized with the same canonical release ledger. Generated NPC identities, emotion/deterrence, dogs, health, all-water Possibility State, and generated regional settlement ecology remain explicitly unfinished rather than hidden behind inert controls.
