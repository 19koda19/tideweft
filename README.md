...
- **Footing and physical parcels:** stability now responds to grade, roughness, moisture, water, current, wind, turning, load, footwear, and BRACE. It is not a second stamina bar: dry support performs corrective recovery even while resisting crosswind, while deep water remains pure-loss footing. BRACE now buys meaningful crossing distance without erasing the need for tools or a prepared line. Hazardous entries deterministically stumble or fall, briefly alter the courier's color and silhouette, speak a tiny Atari-like callout, damage one exact cargo lot, and can separate persistent parcels that drift, tumble, weather, save, reload, and remain recoverable.
- **Responsive river recovery:** desktop Shift now braces even when the document body or HUD has focus, with immediate BRACING copy and a color-independent planted marker in Chart and Relief. Overlapping canvas, global Shift, and touch holds cannot cancel one another. Physical parcel evidence keeps a compact hash-chained recent tail instead of re-sealing thousands of records every movement beat; ordinary far-away parcels leave the render interest radius without being deleted, while an active Promise remains recovery-focused with direction, distance, and motion guidance.
- **Signed horizons:** the courier can now cross the old map edge into deterministic terrain in every signed direction. A bounded center-plus-cardinals stream and one-tile overlap keep water, relief, footing, chart marks, Wayknots, and physical cargo coherent while a floating 98 × 74 field recenters. Version-4 saves resume the exact signed region; old 64 × 48 and 96 × 72 saves migrate into compatibility region `(0,0)`.
...
The mobile/current hotfix replaces the desktop HUD on portrait phones and short landscape phones with compact floating route and vital text over the world, with no card or edge shade. **PROMISES + / PROMISES −** opens a mutually exclusive, full-safe-area scroll workspace whose content remains transparent over one uniform dimmed field; settlement interaction uses the same space, so the two cannot stack. Discovered wet surface in both Chart and Relief views carries sparse direction arrows before entry without exposing unsounded depth. In water at or above **120,000** depth, either empty stamina or empty stability yields control to the same recoverable deterministic sweep path.
...
Each seed now creates one continuous signed terrain world. Compatibility region `(0,0)` preserves the original 96 × 72 tidal field, its well-separated harbor network, 42 named residents, five resource economies, changing weather, shortage-driven promises, and five civic projects. Crossing a horizon streams deterministic neighboring terrain, including negative coordinates, while retaining the exact seed, chart, cargo, and placed field kit. Alpha 0.1's existing 64 × 48 saves migrate into region `(0,0)` without regenerating or discarding their authored world. Generated settlements, actors, and sparse causal opportunities outside `(0,0)` are the next world-simulation slice; empty country is not padded with cloned harbors or free loot. The main loop is:
...
- Signed regional terrain in every direction with exact seam overlap, negative-coordinate support, a bounded five-region mobile stream, floating-origin recentering, sparse durable touched-region state, persistent regional cartography, and clean Chart/Relief camera handoff. The quiet HUD names nonzero regions as `R x,y`; remote Promise and report guidance retains its harbor name, global distance, and bearing.
- Continuous foot/wading/skiff travel with stamina, active bracing, terrain-driven footing and stability, deterministic stumbles/falls, automatically derived Rest/Steady/Swift state, fragile shock, perishable freshness, depth sounding, discovery, visible surface-current direction, emergency camp, swept-current recovery, and infrastructure-enabled rescue.
...
The cargo-environment evaluator is now connected to physical loose parcels: material traits bound impact, wetness, contamination, decay, current/lift response, and magical-water pressure. Parcel identity and hash-chained event evidence persist inside save version 4; the recent detailed tail is compacted to a fixed budget while older evidence folds into its irreversible archive hash. One active regional parcel world and sealed touched inactive worlds share the same exact custody manifest, so leaving and returning neither deletes nor duplicates a parcel. An ordinary far parcel may leave the renderer's 32-tile interest radius, but it is not despawned or rerolled and reappears as the same object when approached. A dropped active Promise remains in the focused projection and becomes a **RECOVER CARGO** objective that names its global direction, distance, and motion; it cannot be delivered or renegotiated until its exact quantity returns to custody. Autonomous loose-parcel drift across a regional seam is not live yet: the object remains in the region where it stopped until the player returns. The separate rock/ladder kernel still remains foundation-only: it derives coherent outcrops, crossing risk/cost, and a finite reusable ladder kit, but production movement does not yet supply those outcrops or ladders. Existing ridge terrain and terrain-driven falls are live; procedural ladder-gated formations, Pannier capacity, shroud/liner protection, crafted-Wayknot deployment, harbor lockers, survival exposure, and upgrades remain staged.
...
| Pointer click / tap | Chart a destination; touch taps route to exact harbors/resources and recover visible parcels only on arrival within reach; tapping a visible horizon edge lets travel continue after the regional recenter; fine-pointer parcel clicks never recover remotely |
...
Holding Shift—or holding **BRACE** on mobile—braces the load without stopping; standing still also restores stability. Pace is read-only: **REST** means still, exhausted, swept, or recovering; **STEADY** is ordinary travel; **SWIFT** appears automatically downhill or when deep current carries the porter with its flow. Completed caches shelter perishable food from freshness loss while you are there. The HUD names the terrain, water, wind, turn, load, and support causes whenever stability changes.
...
Relief 3D travel is camera-relative: after orbiting with J/L, drag, or a two-finger twist, WASD/arrows continue to mean screen-left, screen-right, forward, and back. A pointer-transparent N compass stays north-up in Chart and turns with the Relief camera so world north remains legible without changing any simulation direction. Hidden terrain remains flat possibility for rendering, labels, and pointer picking until it is genuinely discovered; the discovery mesh signature is cached per immutable projection instead of rehashed every animation frame.

Water never becomes an arbitrary invisible wall. Sparse arrows show the direction of moving water on already discovered wet surface, but their fixed shape and spacing reveal no unsounded depth. A sounding pulse marks nearby bathymetry; deeper water spends more stamina, while a Tide sail lowers that cost. Empty stamina on dry ground makes camp. In deep/current water at or above 120,000 depth, either stamina or stability reaching zero yields steering to a deterministic adjacent current path toward the nearest safe bank. Held BRACE materially extends the crossing window but eventually fails under continued deep-current pressure; reaching a dry bank restores a viable recovery path even in crosswind. Cargo quantity remains physically accountable, but the fall may damage or separate exact parcels; loose parcels continue moving with current and grade until recovered. A connected clinic can prevent the sweep while a ferry or Storm kite shortens it. Player-controlled ADRIFT paddling is the next water-recovery slice and is not represented as live here.
...
Each deliberate replacement advances the backward-compatible two-part era/generation version before timestamp/tick comparison, including a safe carry into the next era if the generation counter is saturated. If both counters are already at JavaScript's largest safe integer, the game refuses to wrap them and visibly asks the player to clear Tideweft's stored site data before beginning again. The Alpha 0.3 version-4 session retains the sealed physical-cargo and traversal ledgers from version 3, then adds an independently sealed signed-region stream, sparse durable manifest, regional cartography, and Promise-journey evidence. Its matching outer payload fence rejects a half-completed crossing, wrong active cargo region, stale chart, downgrade, rollback, duplication, or silent deletion before adoption. Version-1, version-2, and version-3 sessions migrate into compatibility region `(0,0)` without losing valid contents; an unreadable or structurally incompatible recorded session is quarantined behind the explicit replacement flow rather than silently loaded or overwritten. New worlds use perpetual `wander`; valid older `drift` and `weave` fields remain readable without regaining timed semantics.
...
`package:desktop` rebuilds the web target and writes an unpacked app under `release/`. The smoke command launches that packaged app with isolated user data, proves the text-only title controls are visibly painted, verifies the secure `app://bundle/` build, opens the canonical Patch Notes from the title, starts compatibility region `(0,0)` inside the floating 98 × 74 Relief window, accepts and physically loads a promise, and binds a real Wayknot through the field-action button. It then reloads a deterministic R1/A3/W5 Tide Harp fixture through production save validation, verifies projection/HUD agreement, sounds a remote tile that the ordinary player-radius pulse cannot reach, round-trips Chart and Relief, exercises held-L camera rotation against the live compass, and checks non-overlapping objective/Promises layouts at 1,440 × 900, 960 × 640, and 927 × 640. At 700 × 640, 320 × 640 portrait, and 844 × 390 landscape it proves the compact HUD defaults closed, keeps all four vital labels visible, exposes touch-size controls and current/safety guidance, opens a full-width scrollable Promises sheet without covering the strip or action dock, exercises Patch Notes through the mobile tutorial and Quiet Hour, follows the visible Quiet Hour → saved title → return path, keeps the inspector mutually exclusive, and closes cleanly again. A separate 360 × 640 probe opens the real non-pausing KIT modal, checks PACK / MAKE / MEND and exact load, verifies all recipe targets and blockers, physically scrolls MAKE, and proves close restores focus. Unless `--no-screenshot` is supplied, the run writes `artifacts/electron-title-smoke.png`, `artifacts/electron-mobile-smoke.png`, and `artifacts/electron-smoke.png`. `make:desktop` creates the platform ZIP.
...
The `0.3.3-alpha.3` source candidate carries gameplay contract 12 and tutorial 13. It separates a broad, softly fading terrain horizon from the shorter exact-detail field used by actors, cargo, labels, live status, events, currents, and interactions. Currently seen uncharted ground and ordinary water appear in both Chart and Relief without becoming durable map knowledge, and hidden persistent systems continue simulating without entering the draw or hit-test paths. [CHANGELOG.md](./CHANGELOG.md), the title and Quiet Hour Patch Notes, and the field manual's **What's New** page are generated from or synchronized with the same canonical release ledger. Player-controlled ADRIFT recovery, audible living-actor contacts, generated NPC identities, emotion/deterrence, dogs, health, and generated regional settlement ecology remain explicitly unfinished rather than hidden behind inert controls.
