...
- **The deliberate return:** every new and resumed estuary now uses the one official ruleset, **A CHALLENGING HARD**. A valid local save resumes automatically; deliberate replacement requires the exact `restartrestartrestart` phrase and a new seed. Offline Patch Notes, visible save-health recovery, longer unladen travel, compact mobile instruments, Relief rotation, and a world-north compass make that single path legible.
...
- **Footing and physical parcels:** stability now responds to grade, roughness, moisture, water, current, wind, turning, load, footwear, and BRACE. Hazardous entries deterministically stumble or fall, briefly alter the courier's color and silhouette, speak a tiny Atari-like callout, damage one exact cargo lot, and can separate persistent parcels that drift, tumble, weather, save, reload, and remain recoverable.
...
3. Travel manually or set a pointer destination; sound uncertain water, read the terrain-driven footing warning, and BRACE through prepared hazards. Pace changes automatically with stillness, recovery, downhill motion, and assisting current.
...
The `0.3.1-alpha.0` source candidate establishes the official **A CHALLENGING HARD** contract, automatic return, generation-safe deliberate restart, persistent storage-failure feedback with fresh-state retry, 18.000 shared capacity, unladen stamina rebalance, mobile vital labels, Relief spin/compass controls, and one canonical offline release ledger. [CHANGELOG.md](./CHANGELOG.md), the title and Quiet Hour Patch Notes, and the field manual's **What's New** page are generated from or synchronized with that release contract. The candidate honestly retains compatibility region `(0,0)` while infinite streaming remains in progress.
