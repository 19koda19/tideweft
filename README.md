...
The latest published checkpoint is the **perpetual/mobile/biome preview** at commit `29ea8dc`. It remains untagged: the Alpha 0.2 tag has not moved. Phase 9 introduced the six fixed, reusable Wayknots at `eb12db0` and hardened them at `1bc136e`; those Reed mats, Tide anchors, and Wind knots can be bound or reclaimed with **F**, change authoritative movement and pointer-route costs, appear physically in both views, and form a small **Waychord** where unlike fields overlap.

The next **field ecology / KIT** checkpoint is present in the current source candidate but is not yet the build served by GitHub Pages. It adds visible seed-derived material patches, renewable one-unit gathering, one exact shared pack limit, anywhere-accessible crafting and mending, and persistent condition for crafted gear and the inherited Wayknots. Publication commit, workflow runs, and live asset hashes will be recorded only after that candidate clears its release gate and reaches `main`.
...
The unpublished field ecology / KIT source candidate passes TypeScript, **49 Vitest files / 386 checks**, the production build, the nested `/tideweft/` smoke, a scoped source-secret scan, and the packaged desktop/mobile/KIT gate with no renderer warnings or resource failures. Fresh title, portrait-gameplay, and Relief captures were inspected. The exact candidate assets are `index-CHONaHrC.js` and `index-cSiSqast.css`; no publication commit or workflow run is claimed until the push completes.
...
The live origin is verified at commit `29ea8dc60f309ebc43bcf8c1b567cfacf2bf8f95`; successful CI run `33508654754` and Pages run `33508654540` produced the exact inspected `index-DIX0Efr_.js` and `index-Cc-fErTR.css` assets.

The field ecology / KIT work described above is a local source candidate until the next verified push. The live URL therefore continues to serve the perpetual/mobile/biome checkpoint at the commit above.
