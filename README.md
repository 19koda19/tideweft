...
Each deliberate replacement advances the backward-compatible two-part era/generation version before timestamp/tick comparison, including a safe carry into the next era if the generation counter is saturated. If both counters are already at JavaScript's largest safe integer, the game refuses to wrap them and visibly asks the player to clear Tideweft's stored site data before beginning again. The Alpha 0.3 outer session now uses version 7. It retains the sealed physical-cargo, traversal, Promise-journey, cartography, and pending sensory ledgers and adds canonical dog-ecology, porter-response, and living-actor player-choice roots. The nested travel record still stores the exact origin of the bounded moving frame; older 98 × 74 travel records migrate into the 120 × 120 frame without moving the courier. Supported outer versions 1 through 4 migrate with an empty pending sensory interval, version 5 preserves its already-produced footfalls, splashes, and impacts, and version 6 preserves its existing dog/cargo state while gaining deterministic porter-response and empty player-choice state. The version-7 payload fence rejects a half-completed actor transaction, mismatched provision or Promise ownership, missing/aliased roots, stale chart, downgrade, rollback, duplication, or silent deletion before adoption. Older valid sessions keep their contents and original estuary intact; unreadable or structurally incompatible records are quarantined rather than silently loaded or overwritten. New worlds use perpetual `wander`; valid older `drift` and `weave` fields remain readable without regaining timed semantics.
...
The `0.3.3-alpha.11` source candidate carries gameplay contract 19 and tutorial 21. It keeps Alpha 10's bounded human perception and adds the first complete porter-dog-provision-rain web: exactly one independent generated dog, one existing porter, shared lawful visual/scent evidence, weather condition, knowledge-honest ABOUT, five player choices including a real automatic-route detour, one-unit provision custody and consumption, bounded memory, promotion, and reload continuity. The outer game save advances to version 7; supported versions 1 through 6 migrate without inventing dog history or duplicating food. The nested simulation remains format 4 and `tideweft-sim/6`. [CHANGELOG.md](./CHANGELOG.md), title and Quiet Hour Patch Notes, and the field manual's **What's New** page remain synchronized from one release ledger. Additional dogs, full animal populations, bears, birds, deer, ownership, naming, training, affection, companions, group behavior, environmental evidence, physical human pursuit, distant populations, and the broader biodiversity web remain planned rather than presented as complete.
