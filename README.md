...
Each seed now creates one continuous signed terrain world. Compatibility region `(0,0)` preserves the original 96 × 72 tidal field, its well-separated harbor network, 42 persistent generated human residents, five resource economies, changing weather, shortage-driven promises, and five civic projects. Crossing a horizon streams deterministic neighboring terrain, including negative coordinates, while retaining the exact seed, chart, cargo, and placed field kit. Alpha 0.1's existing 64 × 48 saves migrate into region `(0,0)` without regenerating or discarding their authored world. Generated settlements and populations outside `(0,0)`, dogs, wildlife, companions, and full actor-to-actor ecology remain later vertical slices; empty country is not padded with cloned harbors, people, or free loot. The main loop is:
...
The `0.3.3-alpha.8` source candidate carries gameplay contract 16 and tutorial 18. It adds a deliberately limited persistent-human identity and **ABOUT** vertical slice for the compatibility estuary: settlement residents and route porters can be selected in direct sight, visible condition is separated from learned facts, nearby GREET records a bounded introduction, and weather can change a traveler's condition and route behavior. Restrained text emoticons punctuate observable state without exposing exact emotion values. Actor events enter the player-facing feed only when caused or witnessed at their event-time location. The broad terrain field now reaches 52 tiles while exact detail remains 10. The outer game save stays version 4 while embedded simulation snapshots migrate to identity-aware format 3 and rules 5. [CHANGELOG.md](./CHANGELOG.md), title and Quiet Hour Patch Notes, and the field manual's **What's New** page remain synchronized from one release ledger. Universal regional humans, dogs, bears, birds, deer, companions, ownership, negotiation, deterrence, physical NPC inventories, and cross-actor ecology are still planned rather than presented as complete.
