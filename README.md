...
The `0.3.3-alpha.6` source candidate carries gameplay contract 14 and tutorial 16. It ships player-controlled ADRIFT recovery, magnitude-scaled cross-current, exact mid-river save/reload, and a wide forty-two-tile softly fading terrain horizon while exact actors, cargo, labels, live status, current detail, events, and interactions remain inside a short ten-tile field. Relief 3D now preserves the earthy material beneath flooded marsh and meadow cells while its separate water sheet remains blue. Substantial cover and structures can hide details without deleting the larger terrain silhouette; real elevation and weather still close sight. Hidden persistent systems continue simulating without entering the draw or hit-test paths. [CHANGELOG.md](./CHANGELOG.md), the title and Quiet Hour Patch Notes, and the field manual's **What's New** page are generated from or synchronized with the same canonical release ledger. Audible living-actor contacts, generated NPC identities, emotion/deterrence, dogs, health, all-water Possibility State, and generated regional settlement ecology remain explicitly unfinished rather than hidden behind inert controls.
