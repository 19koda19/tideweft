...
`0.3.3-alpha.29 — What Remains` release described below. Exact gameplay
commit `a0f7b571cf6068c41397ad0b8767347b04b24ac1` passed feature CI run
`34187159628`, main CI run `34187706800`, and Pages run `34187706784`; all five
cache-bypassed live artifacts match the tested local production build
byte-for-byte.

The current repository source candidate is
`0.3.3-alpha.30 — Beyond the Harbor`. It is not yet **LIVE_VERIFIED**. This
candidate opens one deterministic remote temperate-upland/forest-edge source
for wild boar, elk, and gray wolf while preserving the deployed Alpha 29 world
as the migration base.
...
- **Bounded living ecology:** one local habitat patch derives the same nineteen-record roster from stable terrain and seed, with conserved aggregate populations, capped persistent individual representatives, groups, homes, and settlement custody. Shared perception, movement, activity, resource, and materialization owners drive bounded behavior while a social group enters detail atomically. **What Remains** adds no species or actor: a marsh fox already pursuing a currently identified marsh rabbit may injure or kill it only at exact physical contact. That death retires one exact rabbit, removes one population unit while any remaining represented units stay abstract reserve, and creates one stable finite carcass. A fox or fish crow must lawfully see and physically reach that body to feed; each meal consumes one conserved unit, and a fox may guard it. Body and direct event presentation remain limited to current lawful perception. This remains one original-harbor assemblage, not worldwide ecology, general animal mortality, reproduction, population recovery, or a complete bestiary.
...
- **Beyond the Harbor source candidate:** Alpha 30 extends the catalog from nineteen to twenty-two records and places wild boar, elk, and gray wolf in one seed-stable remote temperate-upland/forest-edge source. Eligible individuals form reusable **SOUNDER**, **HERD**, and **PACK** groups through shared habitat, perception, attention, locomotion, materialization, and presentation owners. Wolves may exert role-driven pursuit pressure, but only exact contact with a solitary addressable marsh rabbit can enter the current mortality transaction; grouped elk, deer, and all other group members cannot be harmed. A boar may see, reach, claim, and consume from an existing finite body. Chart, Relief, quick inspection, and ABOUT remain knowledge-honest and shared across desktop and mobile. Voice patterns are foundation-only and inaudible; dog interaction is an intentional no-response; tactical pack combat, cougar, additional bear ecotypes, group mortality, ecological migration, worldwide ecology, and full Wave E remain absent.
...
Each seed now creates one continuous deterministic terrain world. The preserved original 96 × 72 tidal country contains its well-separated harbor network, 42 persistent generated human residents, five resource economies, changing weather, shortage-driven promises, civic projects, two distinct generated dogs, and one bounded habitat-derived nineteen-record living-species roster. Walking beyond that old extent requires no edge action or transition: terrain is prepared ahead, exact negative coordinates work, and the same chart, cargo, route, camera, field kit, and persistent objects continue. Alpha 0.1's existing 64 × 48 saves retain their authored world rather than being regenerated. Released Alpha 29 adds only direct marsh-fox contact injury/death for an exact pursued marsh rabbit and one conserved finite body that a fox or fish crow can lawfully reach and feed from. The Alpha 30 source candidate grows the catalog to twenty-two records and adds one deterministic remote upland source for wild boar, elk, and gray wolf without claiming worldwide populations. Generated distant settlements and worldwide ecology, wider domestic ownership, companions, player/dog/human/broader-animal or group-member mortality, reproduction and population recovery, live decomposition, body drift/drag/harvest, scent and insects, and full actor-to-actor ecology remain later vertical slices; empty country is not padded with cloned harbors, people, animals, or free loot. The main loop is:
...
- One strict versioned species catalog covering exactly twenty-two current source records—the nineteen Alpha 29 records plus wild boar, elk, and gray wolf. Rats, frogs, silversides, and fiddler crabs remain non-addressable aggregates. The three additions occupy one deterministic remote upland source and use shared representation, sensory, role, decision, locomotion, group, item-claim, mortality/body, materialization, and presentation contracts. Wolves can pressure eligible prey through role-driven pursuit, while harmful contact still admits only a solitary addressable rabbit; boars can claim and consume finite existing bodies only after current sight and physical reach. Group-member harm, dog interaction with these species, audible upland calls, tactical pack combat, cougar and additional bear ecotypes, worldwide habitat, ecological migration/reproduction, and the rest of the food web remain absent. Shared invariants, deterministic properties, bounded fuzzing, conservation, representative scenarios, and performance witnesses replace a brittle species-by-species or N² test matrix.
...
The Alpha 30 source candidate advances outer session to version 23 and habitat
...
### Source candidate — Alpha 30 Beyond the Harbor

`0.3.3-alpha.30 — Beyond the Harbor` is the current repository source
candidate, not a **LIVE_VERIFIED** deployment. It adds exactly three catalog
...
group member remain ineligible for injury and death. Wild boars can scavenge an
existing body only after seeing it, physically reaching it, and winning the
same finite conserved claim; they do not create or duplicate body resources.
...
The current source has no dog interaction with these species; that row is an
...
worldwide ecology, and full Wave E remain outside this candidate. CI, packaged
smoke, Pages deployment, exact commit evidence, and live-asset verification are
not claimed until those release gates actually pass.
...
The current Alpha 29 feature is **LIVE_VERIFIED** at exact gameplay commit
`a0f7b571cf6068c41397ad0b8767347b04b24ac1`. Feature CI `34187159628`, main
CI `34187706800`, and Pages `34187706784` succeeded. The first cache-bypassed
fetch matched all five live assets to the tested production build exactly:

| Asset | Bytes | SHA-256 |
| --- | ---: | --- |
| `index.html` | 4,168 | `5ce6377ef91f69f603d3ddd8c99b44a72642b61f39a39b19b2b0b32ba3b20cdf` |
...
| `assets/index-CoFIY65V.js` | 3,211,022 | `69d4bd70786556bb1878b85423a3f86222bded7ca6fa470dc4b9f2b52e2c41a9` |
