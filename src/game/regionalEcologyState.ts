 ...
 * Group-free owners use the core actor's exact saturating coarse clock once
 * step-sensitive cognition and intent have settled through canonical chunks.
 * Owners with social cadence retain bounded chunks throughout, so entrant
 * derivation and authority replay remain byte-identical.
 */
function advanceDormantRegionalPatchToTick(
  ...
  while (patch.updatedAtTick < atTick) {
    if (dormantPatchSupportsExactSinglePass(patch, atTick)) {
...
function dormantPatchSupportsExactSinglePass(
  patch: CoreEcologyAggregatePatchState,
  atTick: number,
): boolean {
  return patch.groups.groups.length === 0
    && patch.populations.every(({ members }) => (
      members.every(({ actor, materialization }) => (
