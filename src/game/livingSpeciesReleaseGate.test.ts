...
describe("Living Weft species release gate", () => {
  ...
  it("keeps one knowledge-honest source gate for the Alpha 30 regional upland trio", () => {
    for (const species of ["wild-boar", "elk", "gray-wolf"] as const) {
      ...
      expect(criterion("sound")?.status).toBe("foundation");
      expect(criterion("dog-interaction")).toMatchObject({
        status: "unimplemented",
        evidenceOwnerIds: [],
      });
      for (const [name, evidenceOwnerId] of [
        ["habitat-placement", "test:alpha30-regional-upland-habitat:v1"],
        ["population-materialization", "test:alpha30-regional-upland-materialization:v1"],
        ["full-coarse-transition", "test:alpha30-regional-upland-materialization:v1"],
        ["save-load", "test:alpha30-body-bearing-save-adoption:v1"],
        ["performance-budget", "test:alpha30-regional-upland-performance:v1"],
      ] as const) {
      ...
      expect(criterion("environmental-evidence")?.status).toBe(
        species === "gray-wolf" ? "foundation" : "unimplemented",
      );
      for (const withheld of [
        ...
        expect(criterion(withheld)).toMatchObject({ status: "unimplemented", evidenceOwnerIds: [] });
      }
    }
  });
  ...
  it("authenticates the bounded Alpha-34 capelin seam without inventing voice, feeding, or deployment", () => {
    const releaseGate = gate("atlantic-capelin");
    const criterion = (name: (typeof LIVING_SPECIES_RELEASE_CRITERIA)[number]) => (
      releaseGate.criteria.find((candidate) => candidate.criterion === name)
    );

    expect(livingSpeciesReadinessReport("atlantic-capelin")).toMatchObject({
      evidenceAuthenticated: true,
      state: "blocked",
      publicReady: false,
      counts: { total: 30 },
    ...
    expect(criterion("environmental-evidence")?.evidenceOwnerIds).toContain(
      "test:alpha34-polar-presentation-invariants:v1",
    );
    expect(criterion("tutorial-truth")).toMatchObject({
      status: "active",
      evidenceOwnerIds: ["ui:tutorial-guide:v44"],
    });
    expect(criterion("patch-note-truth")).toMatchObject({
      status: "active",
      evidenceOwnerIds: ["content:patch-notes-alpha34:v1"],
    });
  });
