...
import { createCoreEcologyGroupSet } from "./coreEcologyGroups";
...
describe("runtime BIO0 ecology persistence", () => {
  it("deterministically creates one resident-bound dog and exact provision lot, then reloads without duplication", async () => {
    ...
    expect(firstEnvelope.version).toBe(17);
    expect(firstRepository.snapshot().payloadVersion).toBe(17);
  ...
  it("migrates a sealed v7 save to one deterministic core ecology root and preserves it", async () => {
    ...
    expect(ecology.derivation.kind).toBe("habitat-v8");
    ...
    if (ecology.derivation.kind !== "habitat-v8") throw new Error("missing domestic-yard habitat");
    const duckHabitat = ecology.derivation.habitat.populations.find(({ species }) => (
      species === "american-black-duck"
    ));
    expect(duckHabitat?.populationUnits).toBeGreaterThanOrEqual(0);
    expect(ecology.populations.some(({ species }) => species === "american-black-duck"))
      .toBe((duckHabitat?.populationUnits ?? 0) > 0);
  ...
  it("migrates a sealed v9 Wave-A save once while preserving actors, groups, and physical cargo", async () => {
    ...
    expect(migrated.derivation.kind).toBe("habitat-v8");
    expect(migrated.groups.groups.filter(({ identity }) => (
      identity.species !== "fish-crow" && identity.species !== "domestic-chicken"
    ...
    if (migrated.derivation.kind !== "habitat-v8") {
      throw new Error("missing domestic-yard habitat");
    }
    const migratedDuckHabitat = migrated.derivation.habitat.populations.find(({ species }) => (
      species === "american-black-duck"
    ));
    expect(migrated.populations.some(({ species }) => species === "american-black-duck"))
      .toBe((migratedDuckHabitat?.populationUnits ?? 0) > 0);
  ...
  it("migrates an officially emitted legacy-fixed v9 lineage without losing history or omitting Settlement Shadows", async () => {
    ...
    expect(migrated.derivation.kind).toBe("legacy-fixed-v1-with-habitat-v8");
  ...
  ])("rejects a resealed current v17 envelope with $label", async ({ tamper }) => {
  ...
  it("rejects a resealed v17 ecology whose rat identity is self-consistent but belongs to another seed", async () => {
...
function waveAHabitatFromCurrentEcology(
  ...
  if (ecology.derivation.kind !== "habitat-v8") {
    throw new Error("current migration fixture omitted habitat-v8 derivation");
  }
...
function waveAV9Record(current: SaveRecord): SaveRecord {
  ...
  const waveA = canonicalizeCoreEcologyPatch({
    ...
    groups: createCoreEcologyGroupSet(coreEcology.groups.groups.filter(({ identity }) => (
      CORE_ECOLOGY_WAVE_A_HABITAT_SPECIES.some((species) => species === identity.species)
    ))),
