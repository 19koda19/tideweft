...
describe("TIDEWEFT field-manual content", () => {
  it("keeps one deterministic, complete page order with globally unique content IDs", () => {
    ...
    expect(TUTORIAL_CONTENT_VERSION).toBe(47);
  ...
  it("provides a real What's New action linked to the canonical offline notes", () => {
    ...
    expect(copy).toContain("Alpha 37 Estuary Surface Break is this local unpublished SOURCE_CANDIDATE");
    expect(copy).toContain("has not been pushed, published, deployed, run through remote CI or Pages, or LIVE_VERIFIED");
    expect(copy).toContain("exact thirty-one-record RegionalEcologyStateV5 and outer-version-29 child");
    expect(copy).toContain("bay anchovy, Atlantic ghost crab, great blue heron, common tern, and osprey as records 32 through 36");
    expect(copy).toContain("Anchovies and ghost crabs remain conserved non-addressable aggregates");
    expect(copy).toContain("heron and osprey are solitary persistent individuals");
    expect(copy).toContain("two to four terns form one group-atomic flock");
    expect(copy).toContain("regional quiet, and exact anchovy prey support decide admission");
    expect(copy).toContain("unsupported estuary country remains honestly empty");
    expect(copy).toContain("shared Tide Table keeps anchovies in usable submerged schooling water");
    expect(copy).toContain("exposes ghost-crab activity on suitable tidal flat without creating exact fish or crabs");
    expect(copy).toContain("Authenticated shared neutral activity lets a heron fly to its tide-depth-safe wading ground");
    expect(copy).toContain("terns and ospreys reuse an air-only surface-opportunity and rest loop");
    expect(copy).toContain("immediate danger still wins");
    expect(copy).toContain("Chart, Relief, quick inspection, and ABOUT reveal only lawful current identities or aggregate evidence");
    expect(copy).toContain("RegionalEcologyStateV6 wraps the exact V5 child");
    expect(copy).toContain("outer save version 30 authenticates one sealed outer-version-29 Alpha-36 save");
    expect(copy).toContain("All six ecology layers share one global group-atomic cap of twenty-four addressable actors");
    expect(copy).toContain("One representative visible-versus-ridge-occluded common-tern and anchovy scenario");
    expect(copy).toContain("does not complete Wave G or Directive 04_1");
    expect(copy).toContain("adds no sound, capture, consumption, mortality, reproduction, full circadian life, or continuous 3D flight physics");
    expect(copy).toContain("Dedicated Wave-G performance and seamless actor-crossing proof remain open");
  });
  ...
  it("teaches hard-only automatic resume and the guarded restart phrase", () => {
    ...
    expect(copy).toContain("exact RegionalEcologyStateV5 Alpha-36 composite");
  ...
  it("marks requested future systems as planned instead of claiming that they affect play", () => {
    ...
    expect(plannedCopy).toContain("current thirty-six-record catalog preserves every Alpha-36 layer and appends one Estuary Surface Break breadth cohort");
    expect(plannedCopy).toContain("conserved bay-anchovy and Atlantic-ghost-crab aggregates");
    expect(plannedCopy).toContain("one solitary great blue heron");
    expect(plannedCopy).toContain("one group-atomic common-tern flock");
    expect(plannedCopy).toContain("one solitary osprey");
    expect(plannedCopy).toContain("three birds require the exact local anchovy substrate");
    expect(plannedCopy).toContain("regional quiet preserve honest absence");
    expect(plannedCopy).toContain("Rats, frogs, silversides, fiddler crabs, pikas, capelin, anchovies, and ghost crabs");
    expect(plannedCopy).toContain("eligible larger wildlife use bounded persistent representatives");
    expect(plannedCopy).toContain("Additional Wave-G cohorts");
    ...
    expect(plannedCopy).toContain("new cohort adds no capture, kill, consumption, mortality, body, player harm, dog harm, or human harm");
    expect(plannedCopy).toContain("Directive 04_1 completion");
  ...
  it("teaches the bounded wildlife crossing without claiming the full bestiary", () => {
    ...
    expect(copy).toContain("great blue heron, common tern, osprey");
    expect(copy).toContain("bay-anchovy sign, or Atlantic-ghost-crab sign");
    ...
    expect(copy).toContain("deer, elk, and mountain goats persist in herds, gulls, fish crows, and common terns in flocks, wild boar in sounders, and gray wolves in packs");
    ...
    expect(copy).toContain("Brown rats, southern leopard frogs, Atlantic silversides, Atlantic marsh fiddler crabs, American pikas, Atlantic capelin, bay anchovies, and Atlantic ghost crabs remain conserved");
    ...
    expect(copy).toContain("Rat, frog, silverside, fiddler-crab, pika, capelin, anchovy, and ghost-crab sign ABOUT is close-only because a population sign is not an actor");
    ...
    expect(copy).toContain("current registry to eight reusable archetypes serving eleven species");
    ...
    expect(copy).toContain("great blue heron adds anchored-wader behavior at one tide-depth-safe wading ground");
    expect(copy).toContain("Gull, common tern, and osprey share aerial-surface-opportunist behavior");
    expect(copy).toContain("grants no swimming, wading, exact fish target, capture, or consumption");
    ...
    expect(copy).toContain("not full circadian life or continuously simulated 3D flight");
    ...
    expect(copy).toContain("cougar, brown-bear, mountain-goat, pika, golden-eagle, capelin, Arctic-fox, harbor-seal, polar-bear, bay-anchovy, Atlantic-ghost-crab, great-blue-heron, common-tern, and osprey voice behavior is not implemented");
    ...
    expect(copy).toContain("Alpine triad, the polar-shore forage school, one cold-shore Arctic fox, the polar-consumer seal-and-bear vertical, and the five-profile Estuary Surface Break cohort");
    ...
    expect(copy).toContain("current thirty-six-record catalog preserves the exact thirty-one-record Alpha-36 prefix");
    expect(copy).toContain("Every base, Alpine, polar-shore, cold-shore, polar-consumer, and breadth owner shares one global group-atomic materialization cap of 24 addressable actors");
    expect(copy).toContain("Alpha 37 opens Wave G with one cohort but does not complete Wave G, Directive 04_1");
    expect(copy).toContain("new mortality, live-prey capture or exact pika, capelin, anchovy, or ghost-crab targets, seal capture, consumption, sea ice, snow behavior");
    expect(copy).toContain("rarer solitary polar bear is eligible only after that exact seal candidate exists");
  ...
  it("teaches the v6 breadth wrapper without rewriting the exact v5 ecology", () => {
    ...
    expect(copy).toContain("Outer save version 30 adds RegionalEcologyStateV6");
    expect(copy).toContain("exact RegionalEcologyStateV5 Alpha-36 composite");
    ...
    expect(copy).toContain("separate version-1 sparse breadth root owns only cohort activations and genuine Estuary Surface Break resident deviations");
