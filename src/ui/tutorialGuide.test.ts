...
describe("TIDEWEFT field-manual content", () => {
  it("keeps one deterministic, complete page order with globally unique content IDs", () => {
    ...
    expect(TUTORIAL_CONTENT_VERSION).toBe(48);
  ...
  it("provides a real What's New action linked to the canonical offline notes", () => {
    ...
    expect(copy).toContain("Alpha 38 Marsh Channel Web is this local unpublished SOURCE_CANDIDATE");
    expect(copy).toContain("has not been pushed, published, deployed, run through remote CI or Pages, or LIVE_VERIFIED");
    expect(copy).toContain("exact thirty-six-record catalog and epoch-1 breadth prefix");
    expect(copy).toContain("Atlantic menhaden, mummichog, grass shrimp, blue crab, greater yellowlegs, belted kingfisher, and double-crested cormorant as records 37 through 43");
    expect(copy).toContain("four fish and crustacean profiles remain conserved non-addressable aggregates");
    ...
    expect(copy).toContain("Yellowlegs require grass shrimp, kingfishers require mummichog, and cormorants require menhaden");
    expect(copy).toContain("unsupported country remains honestly empty");
    expect(copy).toContain("Shared Tide Table and aggregate policy move only conserved units");
    expect(copy).toContain("Yellowlegs reuse anchored wading");
    expect(copy).toContain("kingfishers reuse air-only surface-opportunity and perch behavior");
    expect(copy).toContain("cormorants use a shared diving-waterbird routine");
    ...
    expect(copy).toContain("RegionalEcologyStateV6 and outer save version 30 remain unchanged");
    expect(copy).toContain("append epoch 2 exactly once at their saved tick");
    expect(copy).toContain("one global group-atomic cap of twenty-four addressable actors");
    expect(copy).toContain("clear-versus-ridge-occluded cormorant and menhaden scenario");
    expect(copy).toContain("reaches forty-one of the forty-five core-wildlife profiles");
    expect(copy).toContain("one final four-profile cohort and the Directive 04_1 closure gates remain");
    expect(copy).toContain("adds no sound, capture, consumption, new mortality or bodies, reproduction, full circadian life, or continuously simulated 3D flight");
    expect(copy).toContain("dedicated Wave-G performance and seamless actor-crossing closure evidence remain open");
  });
  ...
  it("marks requested future systems as planned instead of claiming that they affect play", () => {
    ...
    expect(plannedCopy).toContain("current forty-three-record catalog preserves every Alpha-37 layer and appends the epoch-2 Marsh Channel Web cohort");
    expect(plannedCopy).toContain("conserved Atlantic-menhaden, mummichog, grass-shrimp, and blue-crab aggregates");
    expect(plannedCopy).toContain("one group-atomic greater-yellowlegs flock");
    expect(plannedCopy).toContain("one solitary belted kingfisher");
    expect(plannedCopy).toContain("one group-atomic double-crested-cormorant flock");
    expect(plannedCopy).toContain("each bird requires its exact local aggregate substrate");
    expect(plannedCopy).toContain("regional quiet preserve honest absence");
    expect(plannedCopy).toContain("Rats, frogs, silversides, fiddler crabs, pikas, capelin, anchovies, ghost crabs, menhaden, mummichog, grass shrimp, and blue crabs");
    expect(plannedCopy).toContain("eligible larger wildlife use bounded persistent representatives");
    expect(plannedCopy).toContain("One final four-profile Wave-G cohort");
    ...
    expect(plannedCopy).toContain("Directive 04_1 closure");
  ...
  it("teaches the bounded wildlife crossing without claiming the full bestiary", () => {
    ...
    expect(copy).toContain("deer, elk, and mountain goats persist in herds; gulls, fish crows, common terns, greater yellowlegs, and double-crested cormorants in flocks; wild boar in sounders; and gray wolves in packs");
    ...
    expect(copy).toContain("Brown rats, southern leopard frogs, Atlantic silversides, Atlantic marsh fiddler crabs, American pikas, Atlantic capelin, bay anchovies, Atlantic ghost crabs, Atlantic menhaden, mummichog, grass shrimp, and blue crabs remain conserved");
    ...
    expect(copy).toContain("Rat, frog, silverside, fiddler-crab, pika, capelin, anchovy, ghost-crab, menhaden, mummichog, grass-shrimp, and blue-crab sign ABOUT is close-only because a population sign is not an actor");
    ...
    expect(copy).toContain("current registry to nine reusable archetypes serving fourteen species");
    ...
    expect(copy).toContain("grant no exact fish target, capture, or consumption");
    ...
    expect(copy).toContain("every Wave-G species have no new audible Living Voice");
    ...
    expect(copy).toContain("Alpine triad, the polar-shore forage school, one cold-shore Arctic fox, the polar-consumer seal-and-bear vertical, and both Wave-G breadth cohorts");
    ...
    expect(copy).toContain("current forty-three-record catalog preserves the exact thirty-six-record Alpha-37 prefix");
    ...
    expect(copy).toContain("Alpha 38 reaches forty-one of the directive's forty-five core-wildlife profiles");
    expect(copy).toContain("new mortality, live-prey capture or exact aggregate-animal targets, seal capture, consumption, sea ice, snow behavior");
    expect(copy).toContain("rarer solitary polar bear is eligible only after that exact seal candidate exists");
    ...
    expect(copy).toContain("representative visible-versus-ridge-occluded food-web witnesses");
    expect(copy).toContain("Dedicated Wave-G performance and seamless actor-crossing closure evidence remain open");
  ...
  it("teaches the v6 breadth wrapper without rewriting the exact v5 ecology", () => {
    ...
    expect(copy).toContain("separate version-1 sparse breadth root owns append-only cohort activations and genuine resident deviations for Estuary Surface Break at epoch 1 and Marsh Channel Web at epoch 2");
    expect(copy).toContain("exact V5 child still owns capelin, Arctic fox, harbor seal, and polar bear");
    expect(copy).toContain("sealed outer-version-29 Alpha-36 save is authenticated and adopted exactly once into version 30");
    expect(copy).toContain("complete RegionalEcologyStateV5 child and original outer integrity remain the adoption witness");
    expect(copy).toContain("without rewriting, renumbering, redistributing, retiring, or relabeling any base, Alpine, polar-shore, cold-shore, or polar-consumer wildlife");
    expect(copy).toContain("All six ecology layers enter one deterministic group-atomic materialization and conservation commit capped at 24 addressable actors");
    expect(copy).toContain("valid outer-v30 epoch-1 save authenticates before appending epoch 2 exactly once at its saved tick");
    expect(copy).toContain("complete epoch-1 activation, residents, and deviations remain an exact prefix");
