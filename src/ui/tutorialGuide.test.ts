...
describe("TIDEWEFT field-manual content", () => {
  it("keeps one deterministic, complete page order with globally unique content IDs", () => {
    ...
    expect(TUTORIAL_CONTENT_VERSION).toBe(46);
  ...
  it("provides a real What's New action linked to the canonical offline notes", () => {
    ...
    expect(copy).toContain("Alpha 36 Breath Between Tides is this local unpublished SOURCE_CANDIDATE");
    expect(copy).toContain("has not been pushed, published, deployed, run through remote CI or Pages, or LIVE_VERIFIED");
    expect(copy).toContain("exact twenty-nine-record Alpha-35 catalog");
    expect(copy).toContain("harbor seal as record 30 and polar bear as record 31");
    expect(copy).toContain("authenticated foraging-water and dry-haulout pair");
    expect(copy).toContain("rarer solitary bear");
    expect(copy).toContain("shared amphibious activity");
    expect(copy).toContain("shared identity, condition, perception, attention, locomotion, dormant autonomy, projection, and ABOUT owners");
    expect(copy).toContain("conserved nonlethal seal pressure on capelin");
    expect(copy).toContain("polar-bear pursuit with seal flight");
    expect(copy).toContain("occluding terrain removes those observations and responses");
    expect(copy).toContain("No capture, kill, consumption, new mortality, body, player harm, dog harm, item, or cargo effect");
    expect(copy).toContain("Chart, Relief, quick inspection, and ABOUT reveal only lawfully observed");
    expect(copy).toContain("RegionalEcologyStateV5 wraps the exact V4 child");
    expect(copy).toContain("outer save version 29 authenticates one sealed outer-version-28 Alpha-35 save");
    expect(copy).toContain("All five ecology layers share one global group-atomic cap of twenty-four addressable actors");
    expect(copy).toContain("completes bounded Wave-F role coverage, not Wave G or Directive 04_1");
    expect(copy).toContain("no sea ice, snow behavior, reproduction, or audible Living Voice");
  });
  ...
  it("teaches hard-only automatic resume and the guarded restart phrase", () => {
    ...
    expect(copy).toContain("Outer save version 29");
    expect(copy).toContain("RegionalEcologyStateV5");
    expect(copy).toContain("sealed outer-version-28 Alpha-35 save is authenticated and adopted exactly once into version 29");
    expect(copy).toContain("exact RegionalEcologyStateV4 Alpha-35 composite");
    expect(copy).toContain("complete RegionalEcologyStateV4 child and original outer integrity remain the adoption witness");
    expect(copy).toContain("without rewriting, renumbering, redistributing, retiring, or relabeling any base, Alpine, polar-shore, or cold-shore wildlife");
    expect(copy).toContain("All five ecology layers enter one deterministic group-atomic materialization and conservation commit capped at 24 addressable actors");
    expect(copy).toContain("cannot reroll a regional, Alpine, polar-shore, cold-shore, or polar-consumer lineage");
    expect(copy).toContain("Pristine signed-region, Alpine, polar-shore, cold-shore, and polar-consumer baselines are rederived instead of saved");
    expect(copy).toContain("Alpine adoption, Coldwater Glint adoption, Alpha-35 adoption, working-animal transition, and domestic recovery remain exact and cannot replay");
  });
  ...
  it("marks requested future systems as planned instead of claiming that they affect play", () => {
    ...
    expect(plannedCopy).toContain("current thirty-one-record catalog preserves the signed-region base, Alpine, polar-shore, and Arctic-fox layers");
    expect(plannedCopy).toContain("adds one sparse polar-consumer sibling");
    expect(plannedCopy).toContain("capelin-backed cold saline shore");
    expect(plannedCopy).toContain("one solitary harbor seal with an exact foraging-water and dry-haulout pair");
    expect(plannedCopy).toContain("rarer solitary polar bear only where that seal candidate exists");
    ...
    expect(plannedCopy).toContain("eligible larger wildlife use bounded persistent representatives");
    expect(plannedCopy).toContain("Additional wildlife beyond this bounded roster");
    ...
    expect(plannedCopy).toContain("Harbor seals and polar bears add no capture, kill, consumption, mortality, body, player harm, dog harm, or human harm");
    expect(plannedCopy).toContain("Wave-G breadth, Directive 04_1 completion");
  ...
  it("teaches the bounded wildlife crossing without claiming the full bestiary", () => {
    ...
    expect(copy).toContain("deer, elk, and mountain goats persist in herds, gulls and fish crows in flocks, wild boar in sounders, and gray wolves in packs");
    ...
    expect(copy).toContain("Snowy egrets, American black ducks, and habitat-supported North American river otters use persistent individual identities");
    expect(copy).toContain("One global group-atomic plan selects the nearest 24 eligible individuals across every active base, Alpine, polar-shore, cold-shore, and polar-consumer ecology owner");
    ...
    expect(copy).toContain("Brown rats, southern leopard frogs, Atlantic silversides, Atlantic marsh fiddler crabs, American pikas, and Atlantic capelin remain conserved");
    ...
    expect(copy).toContain("Rat, frog, silverside, fiddler-crab, pika, and capelin sign ABOUT is close-only because a population sign is not an actor");
    ...
    expect(copy).toContain("Grouped elk, grouped deer, and every other social-group member cannot be injured or killed");
    expect(copy).toContain("Brown bear has no live-prey pursuit or harmful contact");
    expect(copy).toContain("species-directed dog interaction with wild boar, elk, gray wolf, cougar, brown bear, mountain goat, pika, golden eagle, capelin, harbor seal, and polar bear remains an intentional no-response");
    expect(copy).toContain("dog or black bear can become the more urgent pressure");
    expect(copy).toContain("Boar, elk, and wolf voice patterns are foundation-only and not audible");
    expect(copy).toContain("cougar, brown-bear, mountain-goat, pika, golden-eagle, capelin, Arctic-fox, harbor-seal, and polar-bear voice behavior is not implemented");
    ...
    expect(copy).toContain("complete evidence tracking");
    expect(copy).toContain("complete sound, general scent propagation");
    ...
    expect(copy).toContain("Alpine triad, the polar-shore forage school, one cold-shore Arctic fox, and the polar-consumer seal-and-bear vertical");
    expect(copy).toContain("role, perception, movement, group, aggregate, ridge-activity, tide, amphibious-activity, physical-item, plural-custody, typed-home, working-assignment, recovery, mortality, body, and presentation contracts");
    expect(copy).toContain("current thirty-one-record catalog preserves the exact signed-region base");
    expect(copy).toContain("Every base, Alpine, polar-shore, cold-shore, and polar-consumer owner shares one global group-atomic materialization cap of 24 addressable actors");
    expect(copy).toContain("completes the directive's bounded Wave-F role coverage, not Wave G, Directive 04_1");
    expect(copy).toContain("new mortality, live-prey capture or exact pika or capelin targets, seal capture, consumption, sea ice, snow behavior");
    expect(copy).toContain("harbor seal; a rarer solitary polar bear exists only where that exact seal candidate exists");
    expect(copy).toContain("lawfully perceived domestic dog can apply generic pressure to an Arctic fox");
  ...
  it("teaches the v5 polar-consumer wrapper without rewriting the exact v4 ecology", () => {
    const saves = tutorialSectionById("saves-and-quiet-hour");
    const copy = saves?.steps.map((step) => step.body).join(" ") ?? "";
    expect(copy).toContain("Outer save version 29 adds RegionalEcologyStateV5");
    expect(copy).toContain("exact RegionalEcologyStateV4 Alpha-35 composite");
    expect(copy).toContain("complete V4 base, Alpine, polar-shore, and cold-shore lineage");
    ...
    expect(copy).toContain("separate sparse polar-consumer sibling owns only harbor-seal and polar-bear resident deviations");
    expect(copy).toContain("exact V4 child still owns Arctic fox and capelin");
    expect(copy).toContain("sealed outer-version-28 Alpha-35 save is authenticated and adopted exactly once into version 29");
    expect(copy).toContain("complete RegionalEcologyStateV4 child and original outer integrity remain the adoption witness");
    expect(copy).toContain("without rewriting, renumbering, redistributing, retiring, or relabeling any base, Alpine, polar-shore, or cold-shore wildlife");
    expect(copy).toContain("All five ecology layers enter one deterministic group-atomic materialization and conservation commit capped at 24 addressable actors");
    ...
    expect(copy).toContain("existing Open Country Ledger disposition, Alpine adoption, Coldwater Glint adoption, Alpha-35 adoption, working-animal transition, and domestic recovery remain exact and cannot replay");
  });
