...
describe("TIDEWEFT field-manual content", () => {
  it("keeps one deterministic, complete page order with globally unique content IDs", () => {
    ...
    expect(TUTORIAL_CONTENT_VERSION).toBe(45);
  ...
  it("provides a real What's New action linked to the canonical offline notes", () => {
    ...
    expect(copy).toContain("Alpha 35 is this unpublished local candidate");
    expect(copy).toContain("has not been pushed, deployed, or LIVE_VERIFIED");
    expect(copy).toContain("exact twenty-eight-record Coldwater Glint catalog to twenty-nine records");
    expect(copy).toContain("one solitary addressable Arctic fox");
    expect(copy).toContain("exact already-admitted Atlantic-capelin forage substrate");
    expect(copy).toContain("shared identity, senses, attention, actor locomotion, condition, dormant autonomy, evidence, projection, and ABOUT owners");
    expect(copy).toContain("lawfully perceived domestic dog can become generic pressure");
    expect(copy).toContain("without a dog-and-fox pair script or invisible deterrence radius");
    expect(copy).toContain("generic nonlethal pressure that lawfully tightens or redistributes the conserved aggregate");
    expect(copy).toContain("terrain occlusion prevents both observation and response");
    expect(copy).toContain("No exact fish is selected, caught, consumed, injured, killed, or turned into a body, item, or cargo resource");
    expect(copy).toContain("Chart, Relief, quick inspection, and ABOUT disclose only current lawful");
    expect(copy).toContain("exact RegionalEcologyStateV3 child is wrapped by version 4");
    expect(copy).toContain("outer save version 28 authenticates one sealed outer-version-27 Coldwater Glint save");
    expect(copy).toContain("one global group-atomic materialization and conservation commit");
    expect(copy).toContain("not complete Wave F, Wave G, or Directive 04_1");
    expect(copy).toContain("no seals, polar bears, new seabirds, snow or ice behavior, new mortality or capture, reproduction, audible Living Voice");
  });
  ...
  it("teaches hard-only automatic resume and the guarded restart phrase", () => {
    ...
    expect(copy).toContain("Outer save version 28");
    expect(copy).toContain("regional ecology root version 4");
    expect(copy).toContain("sealed outer-version-27 Coldwater Glint save is authenticated and adopted exactly once");
    expect(copy).toContain("every earlier population, actor, aggregate, group, item, Promise, home, relationship");
    expect(copy).toContain("complete RegionalEcologyStateV3 child and original outer integrity remain the adoption witness");
    expect(copy).toContain("without rewriting, renumbering, redistributing, retiring, or relabeling any base, Alpine, or polar-shore wildlife");
    expect(copy).toContain("All four ecology layers enter one deterministic group-atomic materialization and conservation commit capped at 24 addressable actors");
    expect(copy).toContain("cannot reroll a regional, Alpine, polar-shore, or cold-shore lineage");
    expect(copy).toContain("Pristine signed-region, Alpine, polar-shore, and cold-shore baselines are rederived instead of saved");
    expect(copy).toContain("Alpine adoption, Coldwater Glint adoption, working-animal transition, and domestic recovery remain exact and cannot replay");
  });
  ...
  it("marks requested future systems as planned instead of claiming that they affect play", () => {
    ...
    expect(plannedCopy).toContain("current twenty-nine-record catalog preserves the exact existing signed-region ecology");
    ...
    expect(plannedCopy).toContain("one solitary Arctic fox through a cold-shore sibling only where that exact admitted forage substrate supports it");
    expect(plannedCopy).toContain("honest absence remains valid");
    expect(plannedCopy).toContain("Each chicken, domestic goat, mountain goat, dog, eagle, and Arctic-fox representative remains an individual");
    ...
    expect(plannedCopy).toContain("Alpine trio, capelin, Arctic fox, brown bears, players, dogs, humans, social-group members, and broader animals cannot enter that harmful path");
    expect(plannedCopy).toContain("remaining polar breadth, complete Waves F and G");
  ...
  it("teaches the bounded wildlife crossing without claiming the full bestiary", () => {
    ...
    expect(copy).toContain("One global group-atomic plan selects the nearest 24 eligible individuals across every active base, Alpine, polar-shore, and cold-shore ecology owner");
    ...
    expect(copy).toContain("SMALL ANIMAL, UNKNOWN CANID, UNKNOWN SMALL CANID, UNKNOWN LARGE CAT, or LARGE BEAR");
    ...
    expect(copy).toContain("Grouped elk, grouped deer, and every other social-group member cannot be injured or killed");
    expect(copy).toContain("Brown bear has no live-prey pursuit or harmful contact");
    expect(copy).toContain("species-directed dog interaction with wild boar, elk, gray wolf, cougar, brown bear, mountain goat, pika, golden eagle, and capelin remains an intentional no-response");
    expect(copy).toContain("dog or black bear can become the more urgent pressure");
    expect(copy).toContain("Boar, elk, and wolf voice patterns are foundation-only and not audible");
    expect(copy).toContain("cougar, brown-bear, mountain-goat, pika, golden-eagle, capelin, and Arctic-fox voice behavior is not implemented");
    ...
    expect(copy).toContain("Alpine triad, the polar-shore forage school, and one cold-shore Arctic fox");
    ...
    expect(copy).toContain("current twenty-nine-record catalog preserves the exact signed-region base");
    expect(copy).toContain("Every base, Alpine, polar-shore, and cold-shore owner shares one global group-atomic materialization cap of 24 addressable actors");
    expect(copy).toContain("remaining polar breadth, complete Wave F, Wave G, or Directive 04_1");
    expect(copy).toContain("new mortality, live-prey capture or exact pika or capelin targets");
    expect(copy).toContain("does not create distant settlements, storage, livestock homes, recovery networks, remaining polar breadth, complete Wave F, Wave G, or Directive 04_1");
  ...
  it("teaches the v4 cold-shore wrapper without rewriting the exact v3 ecology", () => {
    const saves = tutorialSectionById("saves-and-quiet-hour");
    const copy = saves?.steps.map((step) => step.body).join(" ") ?? "";
    expect(copy).toContain("Outer save version 28 adds regional ecology root version 4");
    expect(copy).toContain("exact RegionalEcologyStateV3 base, Alpine, and polar-shore composite from Coldwater Glint");
    expect(copy).toContain("exact Open Country Ledger lineage");
    ...
    expect(copy).toContain("separate sparse cold-shore sibling owns only the addressable Arctic-fox lineage and its true deviations");
    expect(copy).toContain("exact V3 polar-shore child still owns capelin");
    expect(copy).toContain("sealed outer-version-27 Coldwater Glint save is authenticated and adopted exactly once");
    expect(copy).toContain("complete RegionalEcologyStateV3 child and original outer integrity remain the adoption witness");
    expect(copy).toContain("without rewriting, renumbering, redistributing, retiring, or relabeling any base, Alpine, or polar-shore wildlife");
    expect(copy).toContain("All four ecology layers enter one deterministic group-atomic materialization and conservation commit capped at 24 addressable actors");
    ...
    expect(copy).toContain("existing Open Country Ledger disposition, Alpine adoption, Coldwater Glint adoption, working-animal transition, and domestic recovery remain exact and cannot replay");
  });
