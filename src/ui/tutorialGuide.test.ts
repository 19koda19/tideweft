...
describe("TIDEWEFT field-manual content", () => {
  it("keeps one deterministic, complete page order with globally unique content IDs", () => {
    ...
    expect(TUTORIAL_CONTENT_VERSION).toBe(42);
  ...
  it("provides a real What's New action linked to the canonical offline notes", () => {
    ...
    expect(copy).toContain("Open Country Ledger adds no species");
    expect(copy).toContain("deterministic signed-region habitat, carrying capacity, food or prey support, territory, and density budgets");
    expect(copy).toContain("region may be honestly empty");
    expect(copy).toContain("starting density is measured before materialization");
    expect(copy).toContain("free-ranging domestic cat is habitat-optional");
    expect(copy).toContain("one global group-atomic stable-distance top-K plan capped at twenty-four actors");
    expect(copy).toContain("visual, tidal-activity, and alarm perception across owner boundaries");
    expect(copy).toContain("marsh fox, gray wolf, or cougar exactly contacts a currently identified eligible solitary marsh rabbit");
    expect(copy).toContain("victim owner retires it once and owns the conserved physical body");
    expect(copy).toContain("Outer save 25 adds regional ecology root version 1");
    expect(copy).toContain("retains, redistributes, or explicitly retires compatibility wildlife");
    expect(copy).toContain("Compatibility retirement is a non-death record");
    expect(copy).toContain("catalog remains twenty-four records");
    expect(copy).toContain("Worldwide species breadth, ecological migration behavior, reproduction, population recovery, general tactical mortality, complete scent tracking, and the full bestiary remain outside this release");
  });
  ...
  it("teaches hard-only automatic resume and the guarded restart phrase", () => {
    ...
    expect(copy).toContain("Outer save version 25");
    expect(copy).toContain("regional ecology root version 1");
    expect(copy).toContain("normalized sealed version 24 High Country Shadows save migrates exactly once");
    expect(copy).toContain("every earlier population, actor, group, item, Promise, home, relationship");
    expect(copy).toContain("retains, redistributes, or explicitly retires each compatibility wildlife unit");
    expect(copy).toContain("Compatibility retirement is a persisted non-death tombstone");
    expect(copy).toContain("cannot reroll the regional baseline or compatibility disposition");
    expect(copy).toContain("Pristine signed-region baselines are rederived instead of saved");
  ...
  it("marks requested future systems as planned instead of claiming that they affect play", () => {
    ...
    expect(plannedCopy).toContain("current twenty-four-record catalog now derive across signed regions");
    expect(plannedCopy).toContain("rather than being piled around the starting harbor");
    expect(plannedCopy).toContain("region may be honestly empty");
    ...
    expect(plannedCopy).toContain("brown bears, players, dogs, humans, social-group members, and broader animals cannot enter that harmful path");
  ...
  it("teaches the bounded wildlife crossing without claiming the full bestiary", () => {
    ...
    expect(copy).toContain("deer and elk persist in herds, gulls and fish crows in flocks, wild boar in sounders, and gray wolves in packs");
    ...
    expect(copy).toContain("One global group-atomic plan selects the nearest 24 eligible individuals across all active ecology owners");
    ...
    expect(copy).toContain("does not create distant settlements, storage, livestock homes, or recovery networks");
  ...
  it("teaches the current regional adoption without rewriting older ecology", () => {
    const saves = tutorialSectionById("saves-and-quiet-hour");
    const copy = saves?.steps.map((step) => step.body).join(" ") ?? "";
    expect(copy).toContain("Outer save version 25 adds regional ecology root version 1");
