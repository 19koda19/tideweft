...
describe("TIDEWEFT field-manual content", () => {
  ...
  it("teaches the bounded wildlife crossing without claiming the full bestiary", () => {
    ...
    expect(copy).toContain("One global group-atomic plan selects the nearest 24 eligible individuals across every active base and Alpine ecology owner");
    ...
    expect(copy).toContain("Brown rats, southern leopard frogs, Atlantic silversides, Atlantic marsh fiddler crabs, and American pikas remain conserved");
    ...
    expect(copy).toContain("Rat, frog, silverside, fiddler-crab, and pika sign ABOUT is close-only because a population sign is not an actor");
