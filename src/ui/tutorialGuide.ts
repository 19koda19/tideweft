  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        audience: "all",
        title: "First light reaches the field",
        body: "Alpha 41 First Light is a local unpublished source candidate. The single saved clock now eases Chart and Relief through Dawn, Day, Dusk, and Night, while one physical outdoor-light field filters that phase through weather, terrain, cover input, and completed settlement beacons. Exact actors, items, names, and actions become harder to resolve in darkness without erasing the larger ground shape; low-light adaptation preserves only a nearby field, and a ridge can block an earned beacon lamp. Player disclosure, wildlife sight, aggregate activity, exposed food, keeper-to-dog sight, and new human observations consume that same result rather than separate terrain-color guesses. A completed beacon's observation-gated pool is visible on current ground and water in both views without becoming a disclosure overlay or durable map memory. Fresh worlds begin at 07:00, while existing saves keep their exact recorded tick and outer save version 30. Water remains blue through zoom and Relief lighting. No renderer, actor system, region, device clock, or timezone owns a second clock; closing the game adds no offline time. This candidate has not been pushed, deployed, run through remote CI or Pages, or LIVE_VERIFIED. Fire and lantern light, independent canopy/interior cover, complete actor schedules, player sleep/wait, Living Voice, new species, and broader mortality remain unfinished; The Turning Day directive remains open.",
      },
    ],
    callouts: [
      {
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "A directly visible addressable animal—including a seaside sparrow or diamondback terrapin—offers WAIT AND WATCH, ROUTE AROUND THIS SPOT, and LEAVE. WAIT briefly stops the current automatic route. ROUTE AROUND must keep the same destination and find a real line outside the observed animal's area; if that fails, your previous route and choice history remain unchanged. LEAVE closes the observation rather than moving the animal. Aggregate signs, including mosquito and periwinkle evidence, are close-only because a population sign is not an actor; rabbit and fox movement signs are not targetable.",
      },
      {
...
export const TUTORIAL_CONTENT_VERSION = 51 as const;
