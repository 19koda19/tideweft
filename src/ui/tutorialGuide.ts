...
export const TUTORIAL_GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        id: "whats-new-twilight-at-the-marsh-edge",
        audience: "all",
        title: "Twilight at the marsh edge",
        body: "Alpha 49 · Twilight at the Marsh Edge is a local unpublished source candidate. The existing marsh rabbit now composes the shared twilight-active circadian policy with the same saved civil clock. During its stable individual twilight windows, the rabbit may use bounded deterministic local ground activity; outside them, ordinary terrain locomotion carries that same body toward its authenticated habitat cover. Travel remains Awake, Resting begins only after physical arrival, and Asleep begins only after the shared settling interval. Immediate danger, urgent needs, and retained commitments still win, and a stale neutral REST suggestion cannot suppress a newly active twilight preference. The declarative activity registry now contains twelve reusable archetypes serving seventeen species, with truthful circadian-activity capability, circadian-routine scope, and explicit land travel rather than rabbit-only scheduling or pathfinding. Active ground activity creates and consumes no food and never guarantees feeding. The rest site is authenticated habitat cover, not an invented burrow or den. Direct inspection may show only current observable posture and activity, never hidden policy, phase, destination, or internal clock boundary. The existing optional wildlife circadian receipt stores only policy, the authenticated rest-destination receipt, and posture with its entered tick after lawful commitment. Raw save parsing preserves those canonical receipt fields but does not prove the live habitat cover or current body location; full-detail activity projection reauthenticates those world facts before behavioral use. While coarse, the rabbit conserves only an already committed bounded rest bout and does not invent a commute, perception, target, or new decision. Outer save version 32 and RegionalEcologyStateV6 remain unchanged; authenticated historical catalogs retain their exact released rabbit record while the current catalog truthfully names its active crepuscular rhythm. Existing exact-contact rabbit injury, death, and carcass rules are unchanged. The slice adds no rabbit, population unit, density, cargo, item, feeding, capture, attack, mortality, weather driver, tide driver, or opportunity driver. Alpha 48's snowy-egret adaptive routine remains present. This is one representative production crepuscular binding, not validated broad coarse-time advancement, a full catalog schedule, broader autonomous life, or Turning Day closure. The candidate has not been pushed, published, deployed, run through remote CI or Pages, or LIVE_VERIFIED; Alpha 39 remains the latest verified public release.",
      },
    ],
    callouts: [
      {
    ...
    steps: [
      {
        ...
        body: [
          ...
          "Outer save version 32 is current; RegionalEcologyStateV6 remains unchanged inside it. Exact authenticated outer-v30/V6 worlds preserve their nested ecology bytes when adopted through version 31; version 31 then adopts once with no active player recovery action. A valid epoch-2 state authenticates before epoch 3 appends once at its saved tick; earlier epochs remain an exact prefix and reload cannot reroll the result. The egret and rabbit use the existing optional wildlife circadian receipt only after a lawful current projection, so legacy absence remains valid and no migration invents sleep history. That receipt stores policy, the authenticated rest-destination receipt, and posture with its entered tick; actor identity remains separately stable and phase is deterministically rederived. Raw parsing preserves canonical receipt data without proving current location or refuge; full-detail activity reauthenticates before behavioral use. Coarse absence conserves an already committed bounded rest bout without inventing travel or decisions; current drivers are evaluated after rematerialization. Alpha 39 Saltmarsh Small Worlds is the released LIVE_VERIFIED biodiversity checkpoint; Alpha 49 Twilight at the Marsh Edge remains a local unpublished source candidate.",
        ].join(" "),
      },
      {
        ...
        body: "The seven named biomes and their weather, tide, habitat, and magical-water signals are visible now. Rain modifies current visibility, hearing, scent, dog exposure, rat pressure, and frog activity; tide changes conserved aquatic activity and lawful habitat without rerolling populations. The existing snowy egret composes those current tide and lawful aquatic-opportunity signals with the shared adaptive-active clock policy, while the existing marsh rabbit now exercises the shared twilight-active clock policy and terrestrial cover routine; neither binding leaks into unregistered species. The bounded 45 / 47 catalog is not worldwide ecology or a complete bestiary. Shared invariants, signed and extreme-coordinate properties, conservation, performance, seamless crossing, and representative visible-versus-occluded scenarios exercise the reusable scaffold rather than species-by-species or N² pair tests. Fire and lantern light, independent canopy and interior cover, shadow maps, astronomy, sea ice, snow behavior, tactical pack combat, ecological migration, broader mortality and capture, reproduction, population recovery, live-time decomposition, body drift, harvesting, fishing, complete scent and evidence, remaining-human, independent-dog, bonded/player companion, weather-driven bindings, catalog-wide schedules and autonomous home life, validated broad coarse-time advancement, audible Living Voice for the newest species, foliage consumption, and continuous 3D flight remain planned. The Turning Day must still connect those remaining schedules, human and companion continuity, parity/performance proof, and its directive-level gate before Living Voice begins. Absent systems do not alter a delivery behind the HUD's back.",
      },
      {
...
export const TUTORIAL_CONTENT_VERSION = 59 as const;
