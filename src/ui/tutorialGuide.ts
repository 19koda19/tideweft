...
export const TUTORIAL_GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        id: "whats-new-kennel-night",
        audience: "all",
        title: "One working dog has a physical kennel night",
        body: "Alpha 45 · Kennel Night is a local unpublished source candidate. Exactly one existing settlement-custodied working dog now composes the shared clock-driven day-active circadian policy with its real kennel. When rest is preferred, ordinary movement carries that dog to the kennel named by its custody; it remains awake and receives no restorative physiology while travelling. Physical arrival permits Resting and then Asleep after the shared settling interval, and the dog wakes at its stable-ID active boundary. Lawful current danger or strong disturbance, urgent needs and self-preserving intent, and retained investigation or return work still override neutral routine preference. Chart and Relief use its authoritative resting body pose, while directly visible ABOUT inspection can label the current posture Resting or Asleep without revealing the schedule, phase offset, wake threshold, stable database identity, kennel destination ID, custody graph, or assignment internals. The optional routine receipt uses dog actor schema/version 1; outer save version 31, RegionalEcologyStateV6, settlement ecology version 4, and working-animal state version 2 remain unchanged. The independent porter-scene dog is unchanged, and this is not an all-dog, human, bonded/player companion, or player REST/SLEEP routine. Alpha 44's WAIT 10 MIN still advances exactly ten displayed minutes through the same authoritative fixed-step world simulation as ordinary play, grants no special healing or replenishment, and can be interrupted only at a committed fixed-step boundary. Every completed step, elapsed minute, and consequence remains authoritative. Player WAIT is not a pause, REST, SLEEP, Quiet Hour, or the actor ABOUT choice WAIT AND WATCH. Player REST and SLEEP, validated longer or coarse-time acceleration, human and bonded/player companion schedules, broader wildlife bindings, Living Voice, new species, and new mortality remain unfinished. This candidate has not been pushed, published, deployed, run through remote CI or Pages, or LIVE_VERIFIED; Alpha 39 remains the latest verified public release.",
      },
    ],
    callouts: [
      {
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "That one settlement-custodied working dog uses the shared day-active circadian policy. During its night-rest preference it travels physically through ordinary terrain movement to the actual kennel in its custody and stays awake without restorative physiology until arrival. At the kennel it can become Resting and then Asleep; it wakes at its stable-ID active boundary. Lawful danger or strong current disturbance, urgent needs and self-preservation, and retained investigation or return work still outrank the neutral routine. Chart and Relief use its authoritative resting body pose, while directly visible ABOUT inspection may say Resting or Asleep but never reveals schedule timing, phase offset, wake threshold, stable identity, destination ID, custody, or assignment internals. This does not change the independent porter-scene dog or create a bonded/player companion, human schedule, player REST, or player SLEEP.",
      },
      {
...
export const TUTORIAL_CONTENT_VERSION = 55 as const;
